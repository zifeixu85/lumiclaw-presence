...
export function Drawer({open, onOpenChange, title, description, children, footer, className = '', closeLabel = 'Close'}: {open: boolean; onOpenChange: (open: boolean) => void; title: string; description: string; children: ReactNode; footer?: ReactNode; className?: string; closeLabel?: string}) {
  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={changeOpen} modal>
      <DialogPrimitive.Portal>
        <DialogPrimitive.Overlay className="fixed inset-0 z-40 bg-black/35 data-[state=open]:animate-[lc-overlay-in_220ms_ease-out]" />
        <DialogPrimitive.Content aria-modal="true" onCloseAutoFocus={(event) => { event.preventDefault(); restoreFocus.current?.focus(); }} className={`lc-scrollbar fixed inset-y-0 right-0 z-50 flex w-[min(720px,76vw)] flex-col overflow-y-auto border-l border-[var(--lc-line)] bg-[var(--lc-surface)] shadow-[var(--lc-shadow-float)] focus:outline-none data-[state=open]:animate-[lc-drawer-in_240ms_ease-out] ${className}`}>
          <div className="sticky top-0 z-10 flex items-start justify-between gap-6 border-b border-[var(--lc-line)] bg-[var(--lc-surface)] px-7 py-5">
            ...
            <DialogPrimitive.Close className="grid size-9 shrink-0 place-items-center rounded-md border border-[var(--lc-line)] bg-white text-[var(--lc-ink-muted)] hover:text-[var(--lc-ink)]" aria-label={closeLabel}><X aria-hidden size={17} /></DialogPrimitive.Close>
          </div>
