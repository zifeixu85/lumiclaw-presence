...
export const workspaceScreenIds = ['goals', 'campaigns', 'ai-team', 'calendar', 'publish', 'feedback', 'knowledge', 'accounts', 'settings'] as const;
...
export function sectionForLegacyScreen(screen: string): WorkspaceSection {
  ...
  if (screen === 'mission') return 'goals';
  if (screen === 'review') return 'campaigns';
