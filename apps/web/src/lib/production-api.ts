...
export async function createControlledArtifact(execution: MissionExecutionBundle, activationUnitId: string, xMode: 'SINGLE'|'THREAD'): Promise<ArtifactRevisionV3> {
  const response=await requestJson<{revision:ArtifactRevisionV3}>(`/api/v1/artifacts/controlled-fixture`,{method:'POST',headers:exactHeaders(bundleEtagValue(execution),mutationKey('artifact-fixture')),body:JSON.stringify({activationUnitId,bundleId:execution.bundleId,xMode})});return response.revision;
}
export async function editArtifact(revision:ArtifactRevisionV3,payload:ArtifactPayload):Promise<ArtifactRevisionV3>{const response=await requestJson<{revision:ArtifactRevisionV3}>(`/api/v1/artifacts/${revision.id}/edit`,{method:'POST',headers:exactHeaders(artifactRevisionEtag(revision),mutationKey('artifact-edit')),body:JSON.stringify({canonicalDigest:revision.canonicalDigest,payload})});return response.revision;}
export async function requestArtifactRegeneration(revision:ArtifactRevisionV3,reason:string):Promise<void>{await requestJson(`/api/v1/artifacts/${revision.id}/regenerate`,{method:'POST',headers:exactHeaders(artifactRevisionEtag(revision),mutationKey('artifact-regenerate')),body:JSON.stringify({canonicalDigest:revision.canonicalDigest,reason})});}
export async function auditArtifact(revision:ArtifactRevisionV3,result:AuditResult,findings:AuditFinding[]=[]):Promise<ArtifactAuditDecision>{const response=await requestJson<{audit:ArtifactAuditDecision}>(`/api/v1/artifacts/${revision.id}/audits`,{method:'POST',headers:exactHeaders(artifactRevisionEtag(revision),mutationKey('artifact-audit')),body:JSON.stringify({canonicalDigest:revision.canonicalDigest,controlledFixture:true,result,findings})});return response.audit;}
export async function decideArtifact(revision:ArtifactRevisionV3,audit:ArtifactAuditDecision,result:OwnerArtifactDecisionResult,reason:string|null):Promise<ArtifactOwnerDecision>{const response=await requestJson<{decision:ArtifactOwnerDecision}>(`/api/v1/artifacts/${revision.id}/owner-decisions`,{method:'POST',headers:exactHeaders(artifactRevisionEtag(revision),mutationKey('artifact-owner')),body:JSON.stringify({canonicalDigest:revision.canonicalDigest,auditDecisionId:audit.id,auditDecisionDigest:audit.canonicalDigest,result,reason})});return response.decision;}
export async function createManualPublishPackage(revision:ArtifactRevisionV3,audit:ArtifactAuditDecision,decision:ArtifactOwnerDecision):Promise<ManualPublishPackage>{const response=await requestJson<{package:ManualPublishPackage}>('/api/v1/manual-publish-packages',{method:'POST',headers:exactHeaders(ownerDecisionEtag(decision),mutationKey('manual-package')),body:JSON.stringify({artifactRevisionId:revision.id,artifactRevisionDigest:revision.canonicalDigest,auditDecisionId:audit.id,auditDecisionDigest:audit.canonicalDigest,ownerDecisionId:decision.id,ownerDecisionDigest:decision.canonicalDigest})});return response.package;}
export function recordPackageEvent(pack:ManualPublishPackage,kind:'copy-event'|'download-event'|'open-official'):{promise:Promise<{officialUrl:string|null;published:false}>,downloadUrl:string}{const promise=requestJson<{officialUrl:string|null;published:false}>(`/api/v1/manual-publish-packages/${pack.id}/${kind}`,{method:'POST',headers:exactHeaders(packageEtag(pack),mutationKey(`package-${kind}`)),body:'{}'});return {promise,downloadUrl:`/api/v1/manual-publish-packages/${pack.id}/download`};}
