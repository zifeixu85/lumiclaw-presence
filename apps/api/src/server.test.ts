...
function liveSnapshot(request: ModelGenerateRequest<unknown>, errorCode: string | null, value: unknown = null) {
  const config = {temperature: 0, maxTokens: 4000, responseFormat: 'json_object' as const, thinkingMode: 'disabled' as const, timeoutMs: 120000, maxAttempts: 3};
  const inputDigest = sha256Digest({system: request.system, input: request.input, outputSchema: request.outputSchema, config});
  const finishReason = errorCode === 'MODEL_OUTPUT_TRUNCATED' ? 'length' : errorCode === 'MODEL_CONTENT_FILTERED' ? 'content_filter' : errorCode === 'MODEL_TOOL_CALL_FORBIDDEN' ? 'tool_calls' : errorCode === 'MODEL_INFERENCE_RESOURCE_UNAVAILABLE' ? 'insufficient_system_resource' : errorCode === 'MODEL_FINISH_REASON_INVALID' ? null : 'stop';
  return {schemaVersion: 1 as const, id: createUuidV7(now().getTime(), new Uint8Array(10)), missionId: request.missionId, taskId: request.taskId, provider: 'DEEPSEEK' as const, maturity: 'CANARY' as const, model: request.model, response: {id: 'redacted-fixture-response', actualModel: request.model, systemFingerprint: null, finishReason}, config, pricing: {source: 'DEEPSEEK_OFFICIAL_2026-08-04' as const, inputCacheHitUsdPerMillion: 0.0028, inputCacheMissUsdPerMillion: 0.14, outputUsdPerMillion: 0.28, peakMultiplierNotApplied: true as const}, inputDigest, outputDigest: errorCode === null ? sha256Digest(value) : null, tokenUsage: {input: 20, output: 10, cacheHit: 5, cacheMiss: 15, reasoning: 0}, estimatedCostUsd: 0.000004914, latencyMs: 23, attempts: 1, error: errorCode === null ? null : {code: errorCode, retryable: errorCode === 'MODEL_TIMEOUT' || errorCode === 'PROVIDER_UNAVAILABLE' || errorCode === 'MODEL_INFERENCE_RESOURCE_UNAVAILABLE' || /^(?:PROVIDER_HTTP_(?:429|5\d\d))$/u.test(errorCode)}, secretPresent: false as const, createdAt: now().toISOString()};
}
...
describe('M1 Campaign API contract', () => {
  ...
  it('binds a redacted Live provider receipt to the exact domain Task before accepting its Runtime Submit', async () => {
    const app = buildApi({now, runtimeBootstrapSecret: runtimeBootstrapTestSecret, deepseekApiKey: 'conformance-key-that-never-leaves-memory-0001', liveModelProviderFactory: () => ({generateStructured: async (request: ModelGenerateRequest<unknown>) => {
      const value = {frozen: true, assessment: 'Approved Claim and Evidence bindings are frozen for SHADOW review.'}; const config = {temperature: 0, maxTokens: 4000, responseFormat: 'json_object' as const, thinkingMode: 'disabled' as const, timeoutMs: 120000, maxAttempts: 3}; const inputDigest = sha256Digest({system: request.system, input: request.input, outputSchema: request.outputSchema, config}); const outputDigest = sha256Digest(value);
      return {ok: true as const, value, snapshot: {schemaVersion: 1 as const, id: createUuidV7(now().getTime(), new Uint8Array(10)), missionId: request.missionId, taskId: request.taskId, provider: 'DEEPSEEK' as const, maturity: 'CANARY' as const, model: request.model, response: {id: 'redacted-conformance-response', actualModel: request.model, systemFingerprint: null, finishReason: 'stop'}, config, pricing: {source: 'DEEPSEEK_OFFICIAL_2026-08-04' as const, inputCacheHitUsdPerMillion: 0.0028, inputCacheMissUsdPerMillion: 0.14, outputUsdPerMillion: 0.28, peakMultiplierNotApplied: true as const}, inputDigest, outputDigest, tokenUsage: {input: 20, output: 10, cacheHit: 5, cacheMiss: 15, reasoning: 0}, estimatedCostUsd: 0.000004914, latencyMs: 23, attempts: 1, error: null, secretPresent: false as const, createdAt: now().toISOString()}};
  ...
  it.each(['PROVIDER_HTTP_401', 'PROVIDER_HTTP_402', 'PROVIDER_HTTP_404', 'PROVIDER_HTTP_429', 'PROVIDER_HTTP_500', 'PROVIDER_HTTP_502', 'PROVIDER_HTTP_503', 'PROVIDER_HTTP_504', 'MODEL_TIMEOUT', 'PROVIDER_UNAVAILABLE', 'MODEL_RESPONSE_IDENTITY_INVALID', 'MODEL_RETURNED_MODEL_MISMATCH', 'MODEL_OUTPUT_TRUNCATED', 'MODEL_CONTENT_FILTERED', 'MODEL_TOOL_CALL_FORBIDDEN', 'MODEL_INFERENCE_RESOURCE_UNAVAILABLE', 'MODEL_FINISH_REASON_INVALID', 'MODEL_USAGE_INVALID', 'PROVIDER_RESPONSE_INVALID', 'MODEL_JSON_MALFORMED', 'MODEL_SCHEMA_INVALID'])('persists only the allowlisted first-domain provider outcome %s', async (providerOutcomeCode) => {
    ...
    const retryable = ['MODEL_TIMEOUT', 'PROVIDER_UNAVAILABLE', 'MODEL_INFERENCE_RESOURCE_UNAVAILABLE', 'PROVIDER_HTTP_429', 'PROVIDER_HTTP_500', 'PROVIDER_HTTP_502', 'PROVIDER_HTTP_503', 'PROVIDER_HTTP_504'].includes(providerOutcomeCode);
    expect(reopened.json().mission).toMatchObject({state: 'FAILED', runtimeStatus: {failure: {code: providerOutcomeCode, failedTaskId: task.id, retryable}}, modelCalls: [{taskId: task.id, error: {code: providerOutcomeCode, retryable}}], actionGrantCount: 0, connectorCount: 0, externalActionCount: 0});
    expect(JSON.stringify(generated.json())).not.toMatch(/redacted-fixture-response|authorization|bearer|prompt|model content/iu);
  });

  it('binds the production Live broker to persisted non-thinking config and the unchanged 4000-token limit', async () => {
    let brokerRequest: ModelGenerateRequest<unknown> | undefined;
    const provider = {generateStructured: async (request: ModelGenerateRequest<unknown>) => {
      brokerRequest = request;
      const value = {frozen: true, assessment: 'Public-safe deterministic Claim/Evidence freeze.'};
      return {ok: true as const, value, snapshot: liveSnapshot(request, null, value)};
    }} as ModelProvider;
    const {generated, reopened} = await firstLiveDomainAttempt(provider, 'explicit-non-thinking-4000');
    expect(generated.statusCode).toBe(200); expect(brokerRequest).toMatchObject({model: 'deepseek-v4-flash', maxTokens: 4000});
    expect(reopened.json().mission).toMatchObject({modelCalls: [{config: {thinkingMode: 'disabled', maxTokens: 4000, responseFormat: 'json_object'}, error: null}], actionGrantCount: 0, connectorCount: 0, externalActionCount: 0});
  });
