...
describe('SDD-008 guided knowledge API',()=>{
  it('lets an ordinary user save seven recoverable steps, review source excerpts, resolve conflict and approve exact snapshot',async()=>{
    ...
    const deletedSource=overview.sources[0];const removedBytes=knowledgeRepository.removeBlobForTest(deletedSource.blobDigest);expect(removedBytes).toBeDefined();const missingRole=await app.inject({method:'GET',url:`/api/v1/knowledge/snapshots/${approvedDraft.id}/role-context`,headers:{'x-lumiclaw-snapshot-digest':approvedDraft.canonicalDigest}});expect(missingRole.statusCode).toBe(422);expect(missingRole.json().code).toBe('SOURCE_BLOB_MISSING');knowledgeRepository.restoreBlobForTest(deletedSource.blobDigest,removedBytes!);
    const deleted=await app.inject({method:'DELETE',url:`/api/v1/knowledge/sources/${deletedSource.documentId}`,headers:{'if-match':`"knowledge-${overview.session.rowVersion}"`,'idempotency-key':'delete-approved-source-0001'}});expect(deleted.statusCode,deleted.body).toBe(200);overview=deleted.json().overview;expect(overview.sources).toHaveLength(4);expect(overview.approvedHistory[0].sourceRevisionDigests).toHaveLength(5);
    const staleAfterDelete=await app.inject({method:'GET',url:`/api/v1/knowledge/snapshots/${approvedDraft.id}/role-context`,headers:{'x-lumiclaw-snapshot-digest':approvedDraft.canonicalDigest}});expect(staleAfterDelete.statusCode).toBe(412);expect(staleAfterDelete.json().code).toBe('SNAPSHOT_STALE');
