...
import {createManualPublishPackageV4,createMediaAuditDecision,createMediaGenerationSpec,createMediaOwnerDecision,createMediaSetBinding,invalidateMediaGovernance,materializeXhsMediaRevision,mediaCompositionResourceSnapshot,MediaContractError,type ArtifactRevisionV4,type BrandSnapshotBinding,type KnowledgeSnapshotBinding,type MediaArtifactRepository,type MediaProviderGateStatus,type MediaProviderReadiness,type MediaWorkspace} from '@lumiclaw/domain';
...
export function buildApi(options: BuildOptions = {}): FastifyInstance {
  ...
  app.post<{Params:{revisionId:string}}>('/api/v1/media-revisions/:revisionId/owner-decisions',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;if(mediaArtifactRepository===undefined||!isRecord(request.body)||typeof request.body.revisionDigest!=='string'||typeof request.body.auditDecisionId!=='string'||typeof request.body.auditDecisionDigest!=='string'||!['APPROVE','REJECT'].includes(String(request.body.result)))throw new MediaContractError('MEDIA_JOB_STATE_INVALID');const body=request.body;const workspace=await mediaArtifactRepository.getWorkspace(profile.id);const revision=workspace.revisions.find((value)=>value.id===request.params.revisionId&&value.canonicalDigest===body.revisionDigest);const audit=workspace.audits.find((value)=>value.id===body.auditDecisionId&&value.canonicalDigest===body.auditDecisionDigest);if(!revision||!audit)throw new MediaContractError('MEDIA_REVISION_STALE');await requireCurrentMediaGovernance(profile.id,revision,workspace);const decision=createMediaOwnerDecision({ownerId:profile.id,revision,audit,ownerIdentityId:profile.id,result:body.result as 'APPROVE'|'REJECT',createdAt:now().toISOString()});await mediaArtifactRepository.appendOwnerDecision(profile.id,decision,now());return reply.status(201).send({code:decision.result==='APPROVE'?'EXACT_MEDIA_REVISION_APPROVED':'EXACT_MEDIA_REVISION_REJECTED',decision,externalActionCount:0});});
  app.post('/api/v1/media-publish-packages',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;if(mediaArtifactRepository===undefined||!isRecord(request.body))throw new MediaContractError('MEDIA_JOB_STATE_INVALID');const body=request.body;const workspace=await mediaArtifactRepository.getWorkspace(profile.id);const revision=workspace.revisions.find((value)=>value.id===body.revisionId&&value.canonicalDigest===body.revisionDigest);const audit=workspace.audits.find((value)=>value.id===body.auditDecisionId&&value.canonicalDigest===body.auditDecisionDigest);if(!revision||!audit)throw new MediaContractError('MEDIA_REVISION_STALE');if(audit.evidenceMaturity==='CONTROLLED_FIXTURE'&&audit.agentTeamsExecuted===false&&audit.authoritativeForOperations===false&&audit.runtimeReceiptBinding===null)throw new MediaContractError('MEDIA_AUDIT_RUNTIME_AUTHORITY_REQUIRED');const decision=workspace.ownerDecisions.find((value)=>value.id===body.ownerDecisionId&&value.canonicalDigest===body.ownerDecisionDigest);if(!decision)throw new MediaContractError('MEDIA_REVISION_STALE');await requireCurrentMediaGovernance(profile.id,revision,workspace);const parent=await artifactPublishRepository.getRevision(profile.id,revision.parentRevisionId);if(parent?.payload.kind!=='XIAOHONGSHU'||parent.canonicalDigest!==revision.parentRevisionDigest)throw new MediaContractError('MEDIA_PACKAGE_TAMPERED');const textFiles=[{fileName:'title.txt',mediaType:'text/plain' as const,content:parent.payload.title},{fileName:'body.md',mediaType:'text/markdown' as const,content:parent.payload.body},{fileName:'topics.txt',mediaType:'text/plain' as const,content:parent.payload.topics.map((topic)=>`#${topic.replace(/^#/u,'')}`).join(' ')},{fileName:'image-specs.json',mediaType:'application/json' as const,content:JSON.stringify(workspace.specs.filter((value)=>value.artifactRevisionId===revision.parentRevisionId).map((value)=>{const safe={...value};delete (safe as Partial<typeof value>).promptTextPrivateRef;return safe;}),null,2)},{fileName:'media-lineage.json',mediaType:'application/json' as const,content:JSON.stringify({revision:revision.canonicalDigest,mediaSet:revision.mediaSetBinding,brandSnapshot:revision.brandSnapshot,knowledgeSnapshot:revision.knowledgeSnapshot},null,2)}];const pack=createManualPublishPackageV4({ownerId:profile.id,revision,audit,decision,textFiles,createdAt:now().toISOString()});await mediaArtifactRepository.appendPackage(profile.id,pack,now());return reply.status(201).header('ETag',`"media-package-${pack.manifestDigest}"`).send({code:'XHS_MANUAL_PUBLISH_PACKAGE_V4_READY',package:pack,externalState:'UNVERIFIED_EXTERNAL_STATE',published:false});});
...
function sendDomainOrUnavailable(reply: FastifyReply, error: unknown) {
  if(error instanceof MediaContractError){const status=error.code==='MEDIA_SECRET_NOT_CONFIGURED'||error.code==='MEDIA_PROVIDER_PROFILE_STALE'?503:error.code==='MEDIA_REVISION_STALE'||error.code==='MEDIA_LEASE_LOST'||error.code==='MEDIA_PACKAGE_TAMPERED'?412:422;return reply.status(status).send({...errorBody(error.code),details:error.details??error.message});}
