...
import {createManualPublishPackageV4,createMediaAuditDecision,createMediaGenerationSpec,createMediaOwnerDecision,createMediaSetBinding,invalidateMediaGovernance,materializeXhsMediaRevision,mediaCompositionResourceSnapshot,MediaContractError,type ArtifactRevisionV4,type BrandSnapshotBinding,type KnowledgeSnapshotBinding,type MediaArtifactRepository,type MediaWorkspace} from '@lumiclaw/domain';
...
export function buildApi(options: BuildOptions = {}): FastifyInstance {
  ...
  app.get('/api/v1/media/readiness',async(_request,reply)=>{const readiness=options.mediaReadinessProbe===undefined?{state:'NOT_CONFIGURED' as const,configured:false,fingerprint:null,updatedAt:null,profileMaturity:'NOT_RUN_NO_KEY' as const,profileRef:'media-capability://wan-text-to-image/v1',checkedAt:'2026-08-24T00:00:00.000Z',expiresAt:'2026-09-24T00:00:00.000Z'}:await options.mediaReadinessProbe();return reply.header('cache-control','no-store').send({...readiness,purpose:'MEDIA_PROVIDER',controlledFake:{available:true,maturity:'CONTROLLED_FAKE_ENGINEERING',providerEvidence:false},browserSecretInputAllowed:false,modelProviderGateIndependent:true});});
  ...
  app.post<{Params:{revisionId:string}}>('/api/v1/artifacts/:revisionId/media-bindings',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;if(mediaArtifactRepository===undefined)return reply.status(503).send(errorBody('MEDIA_CONTROL_PLANE_UNAVAILABLE'));const source=await requireArtifactRevision(artifactPublishRepository,profile.id,request.params.revisionId);
    ...
    const brandSnapshot=request.body.brandSnapshot as never;const knowledgeSnapshot=request.body.knowledgeSnapshot as never;const activeSnapshots=await options.mediaSnapshotProbe?.();if(activeSnapshots!==undefined&&((brandSnapshot as {id?:string;digest?:string}).id!==activeSnapshots.brandSnapshot.id||(brandSnapshot as {digest?:string}).digest!==activeSnapshots.brandSnapshot.digest||(knowledgeSnapshot as {id?:string;digest?:string}).id!==activeSnapshots.knowledgeSnapshot.id||(knowledgeSnapshot as {digest?:string}).digest!==activeSnapshots.knowledgeSnapshot.digest))throw new MediaContractError('MEDIA_REVISION_STALE');if(finals.length!==selectedIds.size||finals.length!==allSpecs.length||finals.some((value)=>value.compositionSpec.brandSnapshotDigest!==(brandSnapshot as {digest?:string}).digest||value.compositionSpec.knowledgeSnapshotDigest!==(knowledgeSnapshot as {digest?:string}).digest))throw new MediaContractError('MEDIA_SET_INCOMPLETE');const binding=createMediaSetBinding({items:finals,sourceImageSpecs:specs});const revision=materializeXhsMediaRevision({ownerId:profile.id,sourceRevision:{id:source.id,canonicalDigest:source.canonicalDigest,revision:source.revision,producerIdentityId:source.producerIdentityId,payloadDigest:sha256Digest(source.payload)},mediaSetBinding:binding,brandSnapshot,knowledgeSnapshot,createdAt:now().toISOString()});await mediaArtifactRepository.appendRevision(profile.id,revision,now());return reply.status(201).header('ETag',`"media-revision-${revision.canonicalDigest}"`).send({code:'XHS_MEDIA_REVISION_CREATED',revision,requiresIndependentAudit:true});});
...
async function start(): Promise<void> {
  ...
  const blobStore=new LocalContentAddressedBlobStore(blobRoot);const gatewayStatusUrl=process.env.MODEL_GATEWAY_URL;const app = buildApi({repository: new PostgresCampaignRepository(connectionString), shadowRepository: new PostgresShadowMissionRepository(connectionString), localPresenceRepository: new PostgresLocalPresenceRepository(connectionString,blobStore), knowledgeRepository: new PostgresKnowledgeRepository(connectionString,blobStore), goalPlanRepository: new PostgresGoalPlanRepository(connectionString), artifactPublishRepository:new PostgresArtifactPublishRepository(connectionString),mediaArtifactRepository:new PostgresMediaArtifactRepository(connectionString),mediaBlobStore:blobStore,mediaReadinessProbe:readMediaGateStatus,mediaSnapshotProbe:readActiveMediaSnapshots,persistentRuntimeRepository:new PostgresPersistentRuntimeRepository(connectionString),...(gatewayStatusUrl===undefined?{}:{gatewayReadinessProbe:async()=>fetchJsonHealth<GatewayReadiness>(gatewayStatusUrl)}), runtimeImportToken: readComposeSecret('/run/secrets/lumiclaw_runtime_import_token'), deepseekApiKey: readComposeSecret('/run/secrets/deepseek_api_key'), runtimeBootstrapSecret: readComposeSecret('/run/secrets/lumiclaw_runtime_broker_bootstrap')});
  const port = Number.parseInt(process.env.PORT ?? '4000', 10);
  await app.listen({host: '0.0.0.0', port});
}

async function readMediaGateStatus():Promise<MediaReadiness>{const location=process.env.MEDIA_GATE_STATUS_FILE??'/run/lumiclaw/media-gate-status.json';try{const value=JSON.parse(await readFile(location,'utf8')) as MediaReadiness;return {...value,fingerprint:value.fingerprint??null,configured:value.configured===true};}catch{return {state:'NOT_CONFIGURED',configured:false,fingerprint:null,updatedAt:null,profileMaturity:'NOT_RUN_NO_KEY',profileRef:'media-capability://wan-text-to-image/v1',checkedAt:'2026-08-24T00:00:00.000Z',expiresAt:'2026-09-24T00:00:00.000Z'};}}
async function readActiveMediaSnapshots(){const approvedAt=process.env.MEDIA_SNAPSHOT_APPROVED_AT??'2026-08-24T00:00:00.000Z';const brandId=process.env.MEDIA_BRAND_SNAPSHOT_ID;const brandDigest=process.env.MEDIA_BRAND_SNAPSHOT_DIGEST;const knowledgeId=process.env.MEDIA_KNOWLEDGE_SNAPSHOT_ID;const knowledgeDigest=process.env.MEDIA_KNOWLEDGE_SNAPSHOT_DIGEST;if([brandId,brandDigest,knowledgeId,knowledgeDigest].some((value)=>value===undefined))throw new MediaContractError('MEDIA_REVISION_STALE');return {brandSnapshot:{id:brandId!,digest:brandDigest!,state:'APPROVED' as const,approvedAt,expiresAt:null},knowledgeSnapshot:{id:knowledgeId!,digest:knowledgeDigest!,state:'APPROVED' as const,approvedAt,expiresAt:null}};}
