...
export function buildApi(options: BuildOptions = {}): FastifyInstance {
  ...
  app.post<{Params:{revisionId:string}}>('/api/v1/media-revisions/:revisionId/audits',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;if(mediaArtifactRepository===undefined||!isRecord(request.body)||typeof request.body.revisionDigest!=='string'||typeof request.body.auditorIdentityId!=='string'||!['PASS','FAIL','ESCALATE'].includes(String(request.body.result)))throw new MediaContractError('MEDIA_JOB_STATE_INVALID');const body=request.body;const workspace=await mediaArtifactRepository.getWorkspace(profile.id);const revision=workspace.revisions.find((value)=>value.id===request.params.revisionId&&value.canonicalDigest===body.revisionDigest);if(!revision)throw new MediaContractError('MEDIA_REVISION_STALE');await requireCurrentMediaGovernance(profile.id,revision,workspace);const audit=createMediaAuditDecision({ownerId:profile.id,revision,auditorIdentityId:body.auditorIdentityId as string,result:body.result as 'PASS'|'FAIL'|'ESCALATE',createdAt:now().toISOString()});await mediaArtifactRepository.appendAudit(profile.id,audit,now());return reply.status(201).send({code:'INDEPENDENT_MEDIA_AUDIT_RECORDED',audit});});
