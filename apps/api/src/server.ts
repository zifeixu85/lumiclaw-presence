...
import {canonicalize,createManualPublishPackageV4,createMediaAuditDecision,createMediaGenerationSpec,createMediaOwnerDecision,createMediaOwnerVisualReview,createMediaSetBinding,invalidateMediaGovernance,materializeXhsMediaRevision,mediaCompositionResourceSnapshot,MediaContractError,type ArtifactRevisionV4,type BrandSnapshotBinding,type KnowledgeSnapshotBinding,type MediaArtifactRepository,type MediaOwnerVisualReviewChecklistV1,type MediaProviderGateStatus,type MediaProviderReadiness,type MediaWorkspace} from '@lumiclaw/domain';
import {LocalContentAddressedBlobStore,type BlobStore} from '@lumiclaw/blob-store';
import {buildDeterministicZip} from '@lumiclaw/providers';
import {
...
import {PostgresArtifactPublishRepository, PostgresCampaignRepository, PostgresGoalPlanRepository, PostgresKnowledgeRepository, PostgresLocalPresenceRepository,PostgresMediaArtifactRepository,PostgresPersistentRuntimeRepository} from '@lumiclaw/db';
...
export function buildApi(options: BuildOptions = {}): FastifyInstance {
  ...
  const mediaArtifactRepository=options.mediaArtifactRepository;const mediaBlobStore=options.mediaBlobStore;
  const requireCurrentMediaGovernance=async(ownerId:string,revision:ArtifactRevisionV4,workspace:MediaWorkspace)=>{if(workspace.invalidations.some((value)=>value.artifactRevisionId===revision.id))throw new MediaContractError('MEDIA_REVISION_STALE');let reason:Parameters<typeof invalidateMediaGovernance>[0]['reasonCode']|null=null;let currentDigest=mediaCompositionResourceSnapshot.compositorProfileDigest;let active:{brandSnapshot:BrandSnapshotBinding;knowledgeSnapshot:KnowledgeSnapshotBinding}|undefined;try{active=options.mediaSnapshotProbe===undefined?await mediaArtifactRepository?.resolveApprovedGovernance(ownerId,now()):await options.mediaSnapshotProbe(ownerId);}catch(error){if(error instanceof MediaContractError&&['MEDIA_SNAPSHOT_NOT_APPROVED','MEDIA_BRAND_BINDING_STALE','MEDIA_KNOWLEDGE_BINDING_STALE','MEDIA_DIGEST_MISMATCH'].includes(error.code)){reason='MEDIA_KNOWLEDGE_SNAPSHOT_CHANGED';currentDigest=sha256Digest({authority:'UNAVAILABLE',code:error.code});}else throw error;}if(reason===null&&active!==undefined&&(active.knowledgeSnapshot.id!==revision.knowledgeSnapshot.id||active.knowledgeSnapshot.digest!==revision.knowledgeSnapshot.digest)){reason='MEDIA_KNOWLEDGE_SNAPSHOT_CHANGED';currentDigest=active.knowledgeSnapshot.digest;}else if(reason===null&&active!==undefined&&(active.brandSnapshot.id!==revision.brandSnapshot.id||active.brandSnapshot.digest!==revision.brandSnapshot.digest)){reason='MEDIA_BRAND_SNAPSHOT_CHANGED';currentDigest=active.brandSnapshot.digest;}else if(reason===null){for(const item of revision.mediaSetBinding.items){const final=workspace.finalAssets.find((value)=>value.id===item.assetId&&value.canonicalDigest===item.finalAssetDigest);const composition=final?.compositionSpec;if(composition===undefined||composition.templateDigest!==mediaCompositionResourceSnapshot.templateDigest||composition.typographyProfileDigest!==mediaCompositionResourceSnapshot.typographyProfileDigest||composition.compositorProfileDigest!==mediaCompositionResourceSnapshot.compositorProfileDigest||composition.outputProfileDigest!==mediaCompositionResourceSnapshot.outputProfileDigest){reason='MEDIA_COMPOSITION_CHANGED';break;}}}if(reason!==null){const event=invalidateMediaGovernance({ownerId,revision,reasonCode:reason,currentDigest,createdAt:now().toISOString()});if(!workspace.invalidations.some((value)=>value.canonicalDigest===event.canonicalDigest))await mediaArtifactRepository?.appendInvalidation(ownerId,event,now());throw new MediaContractError('MEDIA_REVISION_STALE');}};
  ...
  app.addHook('onClose', async () => { await repository.close(); await shadowRepository.close(); await localPresenceRepository.close(); await knowledgeRepository.close(); await goalPlanRepository.close(); await artifactPublishRepository.close(); await mediaArtifactRepository?.close(); await persistentRuntimeRepository?.close(); });
  ...
  app.get('/api/v1/media/readiness',async(_request,reply)=>{const profile=await localPresenceRepository.getProfile();const readiness=options.mediaReadinessProbe===undefined?notConfiguredMediaReadiness(now()):await options.mediaReadinessProbe(profile?.id??null);return reply.header('cache-control','no-store').send({...readiness,purpose:'MEDIA_PROVIDER',controlledFake:{available:true,maturity:'CONTROLLED_FAKE_ENGINEERING',providerEvidence:false},browserSecretInputAllowed:false,modelProviderGateIndependent:true});});
  app.get('/api/media/readiness',async(_request,reply)=>reply.redirect('/api/v1/media/readiness',308));
  app.get('/api/v1/media',async(_request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;if(mediaArtifactRepository===undefined)return reply.status(503).send(errorBody('MEDIA_CONTROL_PLANE_UNAVAILABLE'));return reply.header('cache-control','no-store').send({code:'GOVERNED_MEDIA_WORKSPACE',workspace:await mediaArtifactRepository.getWorkspace(profile.id),externalState:'UNVERIFIED_EXTERNAL_STATE'});});

  app.post<{Params:{revisionId:string}}>('/api/v1/artifacts/:revisionId/media-generations',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;if(mediaArtifactRepository===undefined||mediaBlobStore===undefined)return reply.status(503).send(errorBody('MEDIA_CONTROL_PLANE_UNAVAILABLE'));const source=await requireArtifactRevision(artifactPublishRepository,profile.id,request.params.revisionId);if(source.platformCode!=='XIAOHONGSHU'||!isRecord(request.body)||request.body.artifactRevisionDigest!==source.canonicalDigest||!['CONTROLLED_FAKE','REAL_PROVIDER'].includes(String(request.body.mode))||!Number.isSafeInteger(request.body.generation)||!Array.isArray(request.body.imageSpecs)||request.body.imageSpecs.length===0)throw new MediaContractError('MEDIA_JOB_STATE_INVALID');const headers=artifactMutationHeaders(request,source.canonicalDigest,artifactRevisionEtag(source));const mode=request.body.mode as 'CONTROLLED_FAKE'|'REAL_PROVIDER';const readiness=mode==='REAL_PROVIDER'?await options.mediaReadinessProbe?.(profile.id):undefined;if(mode==='REAL_PROVIDER'&&readiness?.configured!==true)throw new MediaContractError('MEDIA_SECRET_NOT_CONFIGURED');
    const createdAt=now().toISOString();const specs=[];for(const [index,value] of request.body.imageSpecs.entries()){if(!isExactRecord(value,['altText','overlayCopy','promptText'])||typeof value.promptText!=='string'||typeof value.altText!=='string'||(value.overlayCopy!==null&&typeof value.overlayCopy!=='string'))throw new MediaContractError('MEDIA_JOB_STATE_INVALID');const promptBlob=await mediaBlobStore.put(Buffer.from(value.promptText,'utf8'));specs.push(createMediaGenerationSpec({ownerId:profile.id,artifactRevisionId:source.id,artifactRevisionDigest:source.canonicalDigest,imageSpecPosition:index+1,promptTextPrivateRef:`private-prompt://sha256/${promptBlob.digest}/${promptBlob.size}`,promptText:value.promptText,altText:value.altText,overlayCopy:value.overlayCopy as string|null,createdAt}));}
    const result=await mediaArtifactRepository.enqueue(profile.id,specs,Number(request.body.generation),mode==='CONTROLLED_FAKE'?'media-adapter://controlled-fake/v1':'media-adapter://first-real/v1',headers.idempotencyKey,now());return reply.status(result.replayed?200:202).header('Idempotency-Replayed',String(result.replayed)).send({code:result.replayed?'MEDIA_GENERATION_REPLAYED':'MEDIA_GENERATION_QUEUED',...result,maturity:mode==='CONTROLLED_FAKE'?'CONTROLLED_FAKE':'REAL_PROVIDER_CANARY',providerEvidence:readiness?.providerEvidence===true,externalActionCount:0});
  });
  app.get<{Params:{jobId:string}}>('/api/v1/media-jobs/:jobId',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;if(mediaArtifactRepository===undefined)return reply.status(503).send(errorBody('MEDIA_CONTROL_PLANE_UNAVAILABLE'));const value=await mediaArtifactRepository.getJob(profile.id,request.params.jobId);if(value===undefined)return reply.status(404).send(errorBody('MEDIA_JOB_NOT_FOUND'));return reply.header('cache-control','no-store').header('ETag',`"media-job-${value.job.canonicalDigest}"`).send({code:'MEDIA_JOB_REOPENED',...value});});
  app.get<{Params:{assetId:string}}>('/api/v1/media-assets/:assetId/preview',async(request,reply)=>sendMediaAsset(request,reply,false));
  app.get<{Params:{assetId:string}}>('/api/v1/media-assets/:assetId/download',async(request,reply)=>sendMediaAsset(request,reply,true));
  async function sendMediaAsset(request:FastifyRequest<{Params:{assetId:string}}>,reply:FastifyReply,download:boolean){const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;if(mediaArtifactRepository===undefined||mediaBlobStore===undefined)return reply.status(503).send(errorBody('MEDIA_CONTROL_PLANE_UNAVAILABLE'));const workspace=await mediaArtifactRepository.getWorkspace(profile.id);const asset=workspace.finalAssets.find((value)=>value.id===request.params.assetId);if(asset===undefined)return reply.status(404).send(errorBody('MEDIA_ASSET_NOT_FOUND'));const bytes=await mediaBlobStore.get(asset.blobRef);return reply.header('content-type',asset.mimeType).header('content-length',String(bytes.byteLength)).header('etag',`"sha256-${asset.contentDigest}"`).header('cache-control','private, no-store').header('x-content-type-options','nosniff').header('content-security-policy',"default-src 'none'; sandbox").header('x-lumiclaw-media-role','DELIVERY_COMPOSITE').header('content-disposition',`${download?'attachment':'inline'}; filename="${asset.fileName}"`).send(Buffer.from(bytes));}

  app.post<{Params:{revisionId:string}}>('/api/v1/artifacts/:revisionId/media-bindings',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;if(mediaArtifactRepository===undefined)return reply.status(503).send(errorBody('MEDIA_CONTROL_PLANE_UNAVAILABLE'));const source=await requireArtifactRevision(artifactPublishRepository,profile.id,request.params.revisionId);
    if(!isRecord(request.body)||request.body.artifactRevisionDigest!==source.canonicalDigest||!Array.isArray(request.body.finalAssetIds)||request.body.finalAssetIds.some((value)=>typeof value!=='string')||!isRecord(request.body.brandSnapshot)||!isRecord(request.body.knowledgeSnapshot))throw new MediaContractError('MEDIA_REVISION_STALE');artifactMutationHeaders(request,source.canonicalDigest,artifactRevisionEtag(source));const workspace=await mediaArtifactRepository.getWorkspace(profile.id);const allSpecs=workspace.specs.filter((value)=>value.artifactRevisionId===source.id&&value.artifactRevisionDigest===source.canonicalDigest);const selectedIds=new Set(request.body.finalAssetIds as string[]);const finals=workspace.finalAssets.filter((value)=>selectedIds.has(value.id)).sort((a,b)=>a.position-b.position);const rawById=new Map(workspace.rawAssets.map((value)=>[value.id,value]));const specs=allSpecs.filter((spec)=>finals.some((final)=>rawById.get(final.rawAssetId)?.generationSpecId===spec.id));
    const requestedBrand=request.body.brandSnapshot as {id?:string;digest?:string};const requestedKnowledge=request.body.knowledgeSnapshot as {id?:string;digest?:string};const activeSnapshots=options.mediaSnapshotProbe===undefined?await mediaArtifactRepository.resolveApprovedGovernance(profile.id,now()):await options.mediaSnapshotProbe(profile.id);if(requestedBrand.id!==activeSnapshots.brandSnapshot.id||requestedBrand.digest!==activeSnapshots.brandSnapshot.digest||requestedKnowledge.id!==activeSnapshots.knowledgeSnapshot.id||requestedKnowledge.digest!==activeSnapshots.knowledgeSnapshot.digest)throw new MediaContractError('MEDIA_REVISION_STALE');if(finals.length!==selectedIds.size||finals.length!==allSpecs.length||finals.some((value)=>value.compositionSpec.brandSnapshotDigest!==activeSnapshots.brandSnapshot.digest||value.compositionSpec.knowledgeSnapshotDigest!==activeSnapshots.knowledgeSnapshot.digest))throw new MediaContractError('MEDIA_SET_INCOMPLETE');const binding=createMediaSetBinding({items:finals,sourceImageSpecs:specs});const revision=materializeXhsMediaRevision({ownerId:profile.id,sourceRevision:{id:source.id,canonicalDigest:source.canonicalDigest,revision:source.revision,producerIdentityId:source.producerIdentityId,payloadDigest:sha256Digest(source.payload)},mediaSetBinding:binding,brandSnapshot:activeSnapshots.brandSnapshot,knowledgeSnapshot:activeSnapshots.knowledgeSnapshot,createdAt:now().toISOString()});const stored=await mediaArtifactRepository.appendRevision(profile.id,revision,now());return reply.status(201).header('ETag',`"media-revision-${stored.canonicalDigest}"`).send({code:'XHS_MEDIA_REVISION_CREATED',revision:stored,requiresIndependentAudit:true});});
  app.post<{Params:{revisionId:string}}>('/api/v1/media-revisions/:revisionId/audit-requests',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;if(mediaArtifactRepository===undefined||!isExactRecord(request.body,['revisionDigest'])||typeof request.body.revisionDigest!=='string')throw new MediaContractError('MEDIA_JOB_STATE_INVALID');const body=request.body;const workspace=await mediaArtifactRepository.getWorkspace(profile.id);const revision=workspace.revisions.find((value)=>value.id===request.params.revisionId&&value.canonicalDigest===body.revisionDigest);if(revision===undefined)throw new MediaContractError('MEDIA_REVISION_STALE');await requireCurrentMediaGovernance(profile.id,revision,workspace);const headers=artifactMutationHeaders(request,revision.canonicalDigest,`"media-revision-${revision.canonicalDigest}"`);const result=await mediaArtifactRepository.requestRuntimeAudit(profile.id,revision.id,revision.canonicalDigest,headers.idempotencyKey,now());return reply.status(result.replayed?200:202).header('Idempotency-Replayed',String(result.replayed)).send({code:result.replayed?'MEDIA_A5_AUDIT_REQUEST_REPLAYED':'MEDIA_A5_AUDIT_REQUEST_QUEUED',...result,state:'WAITING_A5',externalActionCount:0,callerSuppliedAuthorityAccepted:false});});
  app.post<{Params:{revisionId:string}}>('/api/v1/media-revisions/:revisionId/audits',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;if(mediaArtifactRepository===undefined||!isExactRecord(request.body,['controlledFixture','revisionDigest'])||typeof request.body.revisionDigest!=='string'||request.body.controlledFixture!==true)throw new MediaContractError('MEDIA_JOB_STATE_INVALID');const body=request.body;const workspace=await mediaArtifactRepository.getWorkspace(profile.id);const revision=workspace.revisions.find((value)=>value.id===request.params.revisionId&&value.canonicalDigest===body.revisionDigest);if(!revision)throw new MediaContractError('MEDIA_REVISION_STALE');await requireCurrentMediaGovernance(profile.id,revision,workspace);const audit=createMediaAuditDecision({ownerId:profile.id,revision,controlledFixture:true,result:'ESCALATE',createdAt:now().toISOString()});await mediaArtifactRepository.appendAudit(profile.id,audit,now());return reply.status(201).send({code:'CONTROLLED_MEDIA_AUDIT_FIXTURE_RECORDED',audit,controlledFixture:true,evidenceMaturity:audit.evidenceMaturity,agentTeamsExecuted:audit.agentTeamsExecuted,authoritativeForOperations:audit.authoritativeForOperations,serverOwnedOutcome:true,nextState:'WAITING_FOR_SDD_007_ACCEPTED_AUDITOR_RECEIPT'});});
  app.post<{Params:{revisionId:string}}>('/api/v1/media-revisions/:revisionId/visual-reviews',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;const keys=['auditDecisionDigest','auditDecisionId','checklist','result','reviewedMediaDigests','revisionDigest'];if(mediaArtifactRepository===undefined||!isExactRecord(request.body,keys)||typeof request.body.revisionDigest!=='string'||typeof request.body.auditDecisionId!=='string'||typeof request.body.auditDecisionDigest!=='string'||!['CONFIRMED','REJECTED'].includes(String(request.body.result))||!isExactRecord(request.body.checklist,['hiddenContentChecked','pixelTextMediaSemanticsConfirmed','renderedTextOcrChecked','visualQualityConfirmed'])||Object.values(request.body.checklist).some((value)=>typeof value!=='boolean')||!Array.isArray(request.body.reviewedMediaDigests)||request.body.reviewedMediaDigests.some((value)=>typeof value!=='string'))throw new MediaContractError('MEDIA_JOB_STATE_INVALID');const body=request.body as {revisionDigest:string;auditDecisionId:string;auditDecisionDigest:string;result:'CONFIRMED'|'REJECTED';checklist:MediaOwnerVisualReviewChecklistV1;reviewedMediaDigests:string[]};const workspace=await mediaArtifactRepository.getWorkspace(profile.id);const revision=workspace.revisions.find((value)=>value.id===request.params.revisionId&&value.canonicalDigest===body.revisionDigest);const audit=workspace.audits.find((value)=>value.id===body.auditDecisionId&&value.canonicalDigest===body.auditDecisionDigest);if(revision===undefined||audit===undefined)throw new MediaContractError('MEDIA_REVISION_STALE');await requireCurrentMediaGovernance(profile.id,revision,workspace);const headers=mediaMutationHeaders(request,mediaAuditEtag(revision.canonicalDigest,audit.canonicalDigest));const review=createMediaOwnerVisualReview({ownerId:profile.id,revision,audit,ownerIdentityId:profile.id,result:body.result,checklist:body.checklist,reviewedMediaDigests:body.reviewedMediaDigests,createdAt:now().toISOString()});const result=await mediaArtifactRepository.appendOwnerVisualReview(profile.id,review,headers.idempotencyKey,now());return reply.status(result.replayed?200:201).header('ETag',mediaVisualReviewEtag(result.review)).header('Idempotency-Replayed',String(result.replayed)).send({code:result.replayed?'MEDIA_OWNER_VISUAL_REVIEW_REPLAYED':result.review.result==='CONFIRMED'?'MEDIA_OWNER_VISUAL_REVIEW_CONFIRMED':'MEDIA_OWNER_VISUAL_REVIEW_REJECTED',...result,externalActionCount:0,auditorPassChanged:false});});
  app.post<{Params:{revisionId:string}}>('/api/v1/media-revisions/:revisionId/owner-decisions',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;const keys=['auditDecisionDigest','auditDecisionId','result','revisionDigest','visualReviewDigest','visualReviewId'];if(mediaArtifactRepository===undefined||!isExactRecord(request.body,keys)||typeof request.body.revisionDigest!=='string'||typeof request.body.auditDecisionId!=='string'||typeof request.body.auditDecisionDigest!=='string'||!['APPROVE','REJECT'].includes(String(request.body.result))||!((typeof request.body.visualReviewId==='string'&&typeof request.body.visualReviewDigest==='string')||(request.body.visualReviewId===null&&request.body.visualReviewDigest===null)))throw new MediaContractError('MEDIA_JOB_STATE_INVALID');const body=request.body as {revisionDigest:string;auditDecisionId:string;auditDecisionDigest:string;visualReviewId:string|null;visualReviewDigest:string|null;result:'APPROVE'|'REJECT'};const workspace=await mediaArtifactRepository.getWorkspace(profile.id);const revision=workspace.revisions.find((value)=>value.id===request.params.revisionId&&value.canonicalDigest===body.revisionDigest);const audit=workspace.audits.find((value)=>value.id===body.auditDecisionId&&value.canonicalDigest===body.auditDecisionDigest);const foundVisual=body.visualReviewId===null?null:[...workspace.visualReviews].reverse().find((value)=>value.id===body.visualReviewId&&value.canonicalDigest===body.visualReviewDigest);if(revision===undefined||audit===undefined||(body.visualReviewId!==null&&foundVisual===undefined))throw new MediaContractError('MEDIA_REVISION_STALE');const visual=foundVisual??null;await requireCurrentMediaGovernance(profile.id,revision,workspace);const expectedEtag=visual===null?mediaAuditEtag(revision.canonicalDigest,audit.canonicalDigest):mediaVisualReviewEtag(visual);const headers=mediaMutationHeaders(request,expectedEtag);const decision=createMediaOwnerDecision({ownerId:profile.id,revision,audit,visualReview:visual,ownerIdentityId:profile.id,result:body.result,createdAt:now().toISOString()});const result=await mediaArtifactRepository.appendOwnerDecision(profile.id,decision,headers.idempotencyKey,now());return reply.status(result.replayed?200:201).header('ETag',mediaOwnerDecisionEtag(result.decision)).header('Idempotency-Replayed',String(result.replayed)).send({code:result.replayed?'MEDIA_OWNER_DECISION_REPLAYED':result.decision.result==='APPROVE'?'EXACT_MEDIA_REVISION_APPROVED':'EXACT_MEDIA_REVISION_REJECTED',...result,externalActionCount:0});});
  app.post('/api/v1/media-publish-packages',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;const keys=['auditDecisionDigest','auditDecisionId','ownerDecisionDigest','ownerDecisionId','revisionDigest','revisionId'];if(mediaArtifactRepository===undefined||!isExactRecord(request.body,keys)||Object.values(request.body).some((value)=>typeof value!=='string'))throw new MediaContractError('MEDIA_JOB_STATE_INVALID');const body=request.body as {revisionId:string;revisionDigest:string;auditDecisionId:string;auditDecisionDigest:string;ownerDecisionId:string;ownerDecisionDigest:string};const workspace=await mediaArtifactRepository.getWorkspace(profile.id);const revision=workspace.revisions.find((value)=>value.id===body.revisionId&&value.canonicalDigest===body.revisionDigest);const audit=workspace.audits.find((value)=>value.id===body.auditDecisionId&&value.canonicalDigest===body.auditDecisionDigest);if(revision===undefined||audit===undefined)throw new MediaContractError('MEDIA_REVISION_STALE');if(audit.evidenceMaturity==='CONTROLLED_FIXTURE'&&audit.agentTeamsExecuted===false&&audit.authoritativeForOperations===false&&audit.runtimeReceiptBinding===null)throw new MediaContractError('MEDIA_AUDIT_RUNTIME_AUTHORITY_REQUIRED');const decision=workspace.ownerDecisions.find((value)=>value.id===body.ownerDecisionId&&value.canonicalDigest===body.ownerDecisionDigest);if(decision===undefined)throw new MediaContractError('MEDIA_REVISION_STALE');await requireCurrentMediaGovernance(profile.id,revision,workspace);const headers=mediaMutationHeaders(request,mediaOwnerDecisionEtag(decision));const parent=await artifactPublishRepository.getRevision(profile.id,revision.parentRevisionId);if(parent?.payload.kind!=='XIAOHONGSHU'||parent.canonicalDigest!==revision.parentRevisionDigest)throw new MediaContractError('MEDIA_PACKAGE_TAMPERED');const textFiles=[{fileName:'title.txt',mediaType:'text/plain' as const,content:parent.payload.title},{fileName:'body.md',mediaType:'text/markdown' as const,content:parent.payload.body},{fileName:'topics.txt',mediaType:'text/plain' as const,content:parent.payload.topics.map((topic)=>`#${topic.replace(/^#/u,'')}`).join(' ')},{fileName:'image-specs.json',mediaType:'application/json' as const,content:canonicalize(workspace.specs.filter((value)=>value.artifactRevisionId===revision.parentRevisionId).map((value)=>{const safe={...value};delete (safe as Partial<typeof value>).promptTextPrivateRef;return safe;}))},{fileName:'media-lineage.json',mediaType:'application/json' as const,content:canonicalize({revision:revision.canonicalDigest,mediaSet:revision.mediaSetBinding,brandSnapshot:revision.brandSnapshot,knowledgeSnapshot:revision.knowledgeSnapshot})}];const pack=createManualPublishPackageV4({ownerId:profile.id,revision,audit,decision,textFiles,createdAt:now().toISOString()});const result=await mediaArtifactRepository.appendPackage(profile.id,pack,headers.idempotencyKey,now());return reply.status(result.replayed?200:201).header('ETag',`"media-package-${result.package.manifestDigest}"`).header('Idempotency-Replayed',String(result.replayed)).send({code:result.replayed?'XHS_MANUAL_PUBLISH_PACKAGE_V4_REPLAYED':'XHS_MANUAL_PUBLISH_PACKAGE_V4_READY',...result,externalState:'UNVERIFIED_EXTERNAL_STATE',published:false});});
  app.get<{Params:{packageId:string}}>('/api/v1/media-publish-packages/:packageId/download',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;if(mediaArtifactRepository===undefined||mediaBlobStore===undefined)return reply.status(503).send(errorBody('MEDIA_CONTROL_PLANE_UNAVAILABLE'));const pack=await mediaArtifactRepository.verifyPackage(profile.id,request.params.packageId,now());const entries=[];for(const file of pack.textFiles)entries.push({fileName:file.fileName,bytes:Buffer.from(file.content),digest:file.digest});for(const file of pack.binaryFiles)entries.push({fileName:file.fileName,bytes:await mediaBlobStore.get(file.blobRef),digest:file.digest});const manifest=Buffer.from(JSON.stringify({schemaVersion:4,packageId:pack.id,manifestDigest:pack.manifestDigest,files:[...pack.textFiles,...pack.binaryFiles].map((file)=>({fileName:file.fileName,digest:file.digest,bytes:file.bytes})),externalState:'UNVERIFIED_EXTERNAL_STATE'},null,2));entries.push({fileName:'manifest.json',bytes:manifest,digest:rawSha256(manifest)});const mediaManifest=Buffer.from(JSON.stringify(pack.mediaManifest,null,2));entries.push({fileName:'media-manifest.json',bytes:mediaManifest,digest:rawSha256(mediaManifest)});const zip=buildDeterministicZip(entries);return reply.header('content-type','application/zip').header('content-disposition',`attachment; filename="lumiclaw-xhs-${pack.id}.zip"`).header('etag',`"media-package-${pack.manifestDigest}"`).header('cache-control','private, no-store').header('x-lumiclaw-external-state','UNVERIFIED_EXTERNAL_STATE').send(zip);});
...
function rawSha256(value:Uint8Array):string{return createHash('sha256').update(value).digest('hex');}
...
function sendDomainOrUnavailable(reply: FastifyReply, error: unknown) {
  if(error instanceof MediaContractError){const status=error.code==='MEDIA_SECRET_NOT_CONFIGURED'||error.code==='MEDIA_PROVIDER_PROFILE_STALE'?503:error.code==='MEDIA_ETAG_REQUIRED'||error.code==='MEDIA_IDEMPOTENCY_KEY_REQUIRED'?428:error.code==='MEDIA_IDEMPOTENCY_KEY_REUSED'||error.code==='MEDIA_OWNER_VISUAL_REVIEW_REQUIRED'||error.code==='MEDIA_OWNER_APPROVAL_REQUIRED'?409:error.code==='MEDIA_REVISION_STALE'||error.code==='MEDIA_LEASE_LOST'||error.code==='MEDIA_PACKAGE_TAMPERED'||error.code==='MEDIA_INPUT_CHANGED'?412:422;return reply.status(status).send({...errorBody(error.code),details:error.details??error.message});}
...
function mediaMutationHeaders(request:FastifyRequest,expectedEtag:string):{idempotencyKey:string}{const idempotencyKey=request.headers['idempotency-key'];if(typeof idempotencyKey!=='string'||idempotencyKey.length<8||idempotencyKey.length>128)throw new MediaContractError('MEDIA_IDEMPOTENCY_KEY_REQUIRED');const ifMatch=request.headers['if-match'];if(typeof ifMatch!=='string')throw new MediaContractError('MEDIA_ETAG_REQUIRED');if(ifMatch!==expectedEtag)throw new MediaContractError('MEDIA_INPUT_CHANGED');return {idempotencyKey};}
function mediaAuditEtag(revisionDigest:string,auditDigest:string){return `"media-audit-${revisionDigest}-${auditDigest}"`;}
function mediaVisualReviewEtag(value:{canonicalDigest:string}){return `"media-visual-review-${value.canonicalDigest}"`;}
function mediaOwnerDecisionEtag(value:{canonicalDigest:string}){return `"media-owner-decision-${value.canonicalDigest}"`;}
...
async function start(): Promise<void> {
  ...
  const blobStore=new LocalContentAddressedBlobStore(blobRoot);const mediaRepository=new PostgresMediaArtifactRepository(connectionString,blobStore);const gatewayStatusUrl=process.env.MODEL_GATEWAY_URL;const app = buildApi({repository: new PostgresCampaignRepository(connectionString), shadowRepository: new PostgresShadowMissionRepository(connectionString), localPresenceRepository: new PostgresLocalPresenceRepository(connectionString,blobStore), knowledgeRepository: new PostgresKnowledgeRepository(connectionString,blobStore), goalPlanRepository: new PostgresGoalPlanRepository(connectionString), artifactPublishRepository:new PostgresArtifactPublishRepository(connectionString),mediaArtifactRepository:mediaRepository,mediaBlobStore:blobStore,mediaReadinessProbe:async(ownerId)=>{const gate=await readMediaGateStatus();return ownerId===null?configuredWithoutCanaryReadiness(gate,new Date()):mediaRepository.getProviderReadiness(ownerId,gate,new Date());},mediaSnapshotProbe:(ownerId)=>mediaRepository.resolveApprovedGovernance(ownerId,new Date()),persistentRuntimeRepository:new PostgresPersistentRuntimeRepository(connectionString),...(gatewayStatusUrl===undefined?{}:{gatewayReadinessProbe:async()=>fetchJsonHealth<GatewayReadiness>(gatewayStatusUrl)}), runtimeImportToken: readComposeSecret('/run/secrets/lumiclaw_runtime_import_token'), deepseekApiKey: readComposeSecret('/run/secrets/deepseek_api_key'), runtimeBootstrapSecret: readComposeSecret('/run/secrets/lumiclaw_runtime_broker_bootstrap')});
  const port = Number.parseInt(process.env.PORT ?? '4000', 10);
  await app.listen({host: '0.0.0.0', port});
}

async function readMediaGateStatus():Promise<MediaProviderGateStatus>{const location=process.env.MEDIA_GATE_STATUS_FILE??'/run/lumiclaw/media-gate-status.json';try{const value=JSON.parse(await readFile(location,'utf8')) as MediaProviderGateStatus;return {fingerprint:typeof value.fingerprint==='string'?value.fingerprint:null,configured:value.configured===true,updatedAt:typeof value.updatedAt==='string'?value.updatedAt:null};}catch{return {configured:false,fingerprint:null,updatedAt:null};}}
function notConfiguredMediaReadiness(at:Date):MediaProviderReadiness{return {state:'NOT_CONFIGURED',configured:false,fingerprint:null,updatedAt:null,profileMaturity:'NOT_RUN_NO_KEY',profileRef:'media-capability://wan-text-to-image/v1',checkedAt:at.toISOString(),expiresAt:null,providerEvidence:false,canaryReceiptDigest:null};}
function configuredWithoutCanaryReadiness(gate:MediaProviderGateStatus,at:Date):MediaProviderReadiness{return !gate.configured||gate.fingerprint===null?notConfiguredMediaReadiness(at):{state:'STARTING',configured:true,fingerprint:gate.fingerprint,updatedAt:gate.updatedAt,profileMaturity:'SECRET_CONFIGURED',profileRef:'media-capability://wan-text-to-image/v1',checkedAt:at.toISOString(),expiresAt:null,providerEvidence:false,canaryReceiptDigest:null};}
