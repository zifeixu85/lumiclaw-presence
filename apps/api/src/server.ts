...
import {isSecretBearingKnowledgeObject, knowledgeEtag, KnowledgeContractError, parseKnowledgeEtag, validateKnowledgeSessionInput, validateSourceCandidates, validateTextSource, type KnowledgeOverview, type KnowledgePlatform, type KnowledgeRepository, type ProfileKind} from '@lumiclaw/domain';
import {accountBindingsFromKnowledge, createOperatingGoalRevision, goalEtag, GoalPlanContractError, planEtag, stableContractId, validateOperatingGoalInput, type AccountOperatingProfileInput, type ContentBrief, type ContentPlanSlot, type GoalPlanRepository, type InvalidationEvent, type MissionBundle, type OperatingGoalInput, type OperatingGoalRevision, type PlanSourceBinding, type PlannerSubmission} from '@lumiclaw/domain';
...
export function buildApi(options: BuildOptions = {}): FastifyInstance {
  ...
  app.put('/api/v1/onboarding/session', async (request, reply) => {
    ...
    await invalidateAfterKnowledgeMutation(goalPlanRepository,profile.id,'KNOWLEDGE_SNAPSHOT_CHANGED',overview,now());
  ...
  app.put<{Params:{platform:string}}>('/api/v1/profiles/accounts/:platform', async (request, reply) => {
    ...
    await invalidateAfterKnowledgeMutation(goalPlanRepository,profile.id,'ACCOUNT_PROFILE_CHANGED',overview,now());
  ...
  app.post('/api/v1/knowledge/sources/text', async (request, reply) => {
    ...
    const overview=await knowledgeRepository.ingestTextSource({ownerId:profile.id,...input},headers.version,headers.idempotencyKey,now());await invalidateAfterKnowledgeMutation(goalPlanRepository,profile.id,'KNOWLEDGE_SNAPSHOT_CHANGED',overview,now()); return reply.status(201).header('ETag',knowledgeEtag(overview.session.rowVersion)).send({code:'SOURCE_TEXT_REVISION_CREATED',overview});
  });
  ...
  app.post('/api/v1/missions/compile',async(request,reply)=>{
    ...
    const inputs=await approvedGoalInputs(knowledgeRepository,profile.id,goal,true);const bundle=compileMissionIntentV2({goal,knowledge:inputs.context,accountProfiles:inputs.bindings});const result=await goalPlanRepository.appendBundle(profile.id,bundle,goal.canonicalDigest,headers.idempotencyKey,now());
  ...
  app.post('/api/v1/content-plans',async(request,reply)=>{
    ...
    const plan=importPlannerSubmissionV2(intent,submission,now().toISOString());const result=await goalPlanRepository.appendPlan(profile.id,plan,null,headers.idempotencyKey,now());return reply.status(result.replayed?200:201).header('ETag',planEtag(result.plan)).header('Idempotency-Replayed',String(result.replayed)).send({code:result.replayed?'CONTENT_PLAN_REPLAYED':'CONTROLLED_PLANNER_SUBMISSION_IMPORTED',plan:result.plan,agentTeamsExecuted:submission.evidenceMaturity==='AGENTTEAMS_RUNTIME',evidenceMaturity:submission.evidenceMaturity});
  });
...
async function approvedGoalInputs(repository: KnowledgeRepository,ownerId:string,goal:OperatingGoalRevision,requireProducerCoverage:boolean){
  ...
  const profiles=Object.values(overview.profiles.accounts).filter((profile):profile is NonNullable<typeof profile>=>profile!==undefined&&profile.kind==='ACCOUNT'&&goal.selectedAccountIds.includes(profile.id)).map((profile)=>({id:profile.id,revision:profile.version,digest:profile.digest,payload:profile.payload as AccountOperatingProfileInput}));
...
async function invalidateAfterKnowledgeMutation(repository:GoalPlanRepository,ownerId:string,reasonCode:InvalidationEvent['reasonCode'],overview:KnowledgeOverview,at:Date){const digest=overview.draft?.canonicalDigest??overview.session.currentSnapshotDigest??'0'.repeat(64);await repository.invalidateBundles(ownerId,reasonCode,digest,at);}
