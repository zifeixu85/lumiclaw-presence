...
export function buildApi(options: BuildOptions = {}): FastifyInstance {
  ...
  app.post<{Params:{revisionId:string}}>('/api/v1/media-revisions/:revisionId/audits',async(request,reply)=>{const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;if(mediaArtifactRepository===undefined||!isExactRecord(request.body,['controlledFixture','result','revisionDigest'])||typeof request.body.revisionDigest!=='string'||request.body.controlledFixture!==true||!['PASS','FAIL','ESCALATE'].includes(String(request.body.result)))throw new MediaContractError('MEDIA_JOB_STATE_INVALID');const body=request.body;const workspace=await mediaArtifactRepository.getWorkspace(profile.id);const revision=workspace.revisions.find((value)=>value.id===request.params.revisionId&&value.canonicalDigest===body.revisionDigest);if(!revision)throw new MediaContractError('MEDIA_REVISION_STALE');await requireCurrentMediaGovernance(profile.id,revision,workspace);const audit=createMediaAuditDecision({ownerId:profile.id,revision,controlledFixture:true,result:body.result as 'PASS'|'FAIL'|'ESCALATE',createdAt:now().toISOString()});await mediaArtifactRepository.appendAudit(profile.id,audit,now());return reply.status(201).send({code:'CONTROLLED_MEDIA_AUDIT_FIXTURE_RECORDED',audit,controlledFixture:true,evidenceMaturity:audit.evidenceMaturity,agentTeamsExecuted:audit.agentTeamsExecuted,authoritativeForOperations:audit.authoritativeForOperations,nextState:'WAITING_FOR_SDD_007_ACCEPTED_AUDITOR_RECEIPT'});});
