...
import {artifactRevisionEtag, auditDecisionEtag, buildManualPublishPackage, controlledAuditFindings, createArtifactAuditDecision, createArtifactOwnerDecision, createArtifactRevision, createControlledProducerSubmission, createOwnerEditRevision, createRegenerationRequest, effectiveArtifactState, effectivePackageState, ownerDecisionEtag, packageEtag, quarantineProducerSubmission, recordPackageHelperEvent, ArtifactContractError, type ArtifactAuditDecision, type ArtifactOwnerDecision, type ArtifactPayload, type ArtifactPublishRepository, type ArtifactRevisionV3, type AuditFinding, type AuditResult, type MissionExecutionBundle, type OwnerArtifactDecisionResult, type ProducerSubmission} from '@lumiclaw/domain';
...
export function buildApi(options: BuildOptions = {}): FastifyInstance {
  ...
  app.post('/api/v1/artifact-submissions',async(request,reply)=>{
    if(!isRecord(request.body)||typeof request.body.inputBindings!=='object'||request.body.inputBindings===null)throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');const submission=request.body as ProducerSubmission;if(submission.evidenceMaturity!=='CONTROLLED_FIXTURE'||submission.agentTeamsExecuted!==false)throw new ArtifactContractError('ARTIFACT_RUNTIME_AUTHORITY_REQUIRED');const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;const bundleId=(submission.inputBindings as {executionBundle?:{id?:unknown}}).executionBundle?.id;if(typeof bundleId!=='string')throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');const bundle=await requireExecution(goalPlanRepository,profile.id,bundleId);const headers=artifactMutationHeaders(request,bundle.canonicalDigest,bundleEtag(bundle));return acceptProducerArtifact(profile.id,submission,bundle,headers.idempotencyKey,artifactPublishRepository,now(),reply);
  });
  ...
  app.post<{Params:{artifactId:string}}>('/api/v1/artifacts/:artifactId/edit',async(request,reply)=>{if(!isExactRecord(request.body,['canonicalDigest','payload'])||typeof request.body.canonicalDigest!=='string')throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;const base=await requireArtifactRevision(artifactPublishRepository,profile.id,request.params.artifactId);const headers=artifactMutationHeaders(request,base.canonicalDigest,artifactRevisionEtag(base));if(request.body.canonicalDigest!==base.canonicalDigest)throw new ArtifactContractError('OWNER_DECISION_STALE');const workspace=await artifactPublishRepository.getWorkspace(profile.id);const head=latestUnitRevision(workspace.revisions,base.activationUnitId);if(head?.id!==base.id)throw new ArtifactContractError('OWNER_DECISION_STALE');const revision=createOwnerEditRevision({ownerId:profile.id,ownerIdentityId:profile.id,base,payload:request.body.payload as ArtifactPayload,revision:base.revision+1,createdAt:now().toISOString()});const result=await artifactPublishRepository.appendRevision(profile.id,revision,base.canonicalDigest,headers.idempotencyKey,now());return reply.status(201).header('ETag',artifactRevisionEtag(result.revision)).header('Idempotency-Replayed',String(result.replayed)).send({code:'OWNER_EDIT_ARTIFACT_REVISION_CREATED',...result,requiresIndependentReaudit:true});});
  ...
  app.post<{Params:{artifactId:string}}>('/api/v1/artifacts/:artifactId/audits',async(request,reply)=>{if(!isExactRecord(request.body,['canonicalDigest','controlledFixture','findings','result'])||typeof request.body.canonicalDigest!=='string'||request.body.controlledFixture!==true||!['PASS','FAIL','ESCALATE'].includes(String(request.body.result))||!Array.isArray(request.body.findings))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');const profile=await requireLocalProfile(localPresenceRepository,reply);if(profile===undefined)return;const revision=await requireArtifactRevision(artifactPublishRepository,profile.id,request.params.artifactId);const headers=artifactMutationHeaders(request,revision.canonicalDigest,artifactRevisionEtag(revision));if(request.body.canonicalDigest!==revision.canonicalDigest)throw new ArtifactContractError('OWNER_DECISION_STALE');const resultCode=request.body.result as AuditResult;const findings=request.body.findings.length===0?controlledAuditFindings(resultCode):request.body.findings as AuditFinding[];const audit=createArtifactAuditDecision({ownerId:profile.id,revision,auditorIdentityId:'controlled-independent-auditor-a5',result:resultCode,findings,evidenceBindings:[revision.inputBindings.knowledgeSnapshot.digest,revision.inputBindings.approvedPlan.digest],createdAt:now().toISOString()});const result=await artifactPublishRepository.appendAudit(profile.id,audit,revision.canonicalDigest,headers.idempotencyKey,now());return reply.status(201).header('ETag',auditDecisionEtag(result.audit)).header('Idempotency-Replayed',String(result.replayed)).send({code:'CONTROLLED_INDEPENDENT_AUDIT_RECORDED',...result,agentTeamsExecuted:false,controlledFixture:true});});
