import {PostgresArtifactPublishRepository,PostgresGoalPlanRepository,PostgresMediaArtifactRepository,PostgresPersistentRuntimeRepository} from '@lumiclaw/db';
...
import {ControlledFakeMediaAdapter,downloadAndIngestProviderResult,EvoLinkMediaAdapter} from '@lumiclaw/providers';
...
if(mediaMode!=='NOT_CONFIGURED'){
  const blobStore=new LocalContentAddressedBlobStore(requiredEnv('BLOB_ROOT'));mediaRepository=new PostgresMediaArtifactRepository(connectionString);const fake=new ControlledFakeMediaAdapter();const secret=mediaMode==='REAL_PROVIDER'?await readSecret(requiredEnv('MEDIA_PROVIDER_SECRET_FILE'),16):null;const provider=secret===null?fake:new EvoLinkMediaAdapter({apiKey:secret});const fingerprint=secret===null?'CONTROLLED_FAKE_NO_SECRET':sha256Digest({purpose:'MEDIA_PROVIDER',secret}).slice(0,16);const expectedAdapterRef=mediaMode==='REAL_PROVIDER'?'media-adapter://first-real/v1':'media-adapter://controlled-fake/v1';const snapshotAt=new Date().toISOString();
  mediaWorker=new MediaArtifactWorker({repository:mediaRepository,blobStore,provider,workerId:`${workerId}-media`,leaseMs,expectedAdapterRef,modelRef:mediaMode==='REAL_PROVIDER'?'media-model://wan-text-to-image/v1':'media-model://controlled-fake/v1',capabilityProfileRef:'media-capability://xhs-1080x1440/v1',providerAdapterSnapshotDigest:sha256Digest({adapter:expectedAdapterRef,checkedAt:'2026-08-24'}),maturity:mediaMode==='REAL_PROVIDER'?'REAL_PROVIDER_CANARY':'CONTROLLED_FAKE',resolvePrompt:async(ref)=>{const match=/^private-prompt:\/\/sha256\/([a-f0-9]{64})\/(\d+)$/u.exec(ref);if(!match)throw new Error('MEDIA_PRIVATE_PROMPT_REF_INVALID');const bytes=await blobStore.get({algorithm:'sha256',digest:match[1]!,size:Number(match[2])});return Buffer.from(bytes).toString('utf8');},resolveResult:async(observation)=>{if(mediaMode==='CONTROLLED_FAKE')return {bytes:await fake.resolve(observation.resultRef!),declaredMime:'image/png'};const downloaded=await downloadAndIngestProviderResult(observation.resultRef!,null,blobStore);return {bytes:await blobStore.get(downloaded.blobRef),declaredMime:downloaded.decoded.mimeType};},ticket:async(scope)=>issueMediaSecretTicket({purpose:'MEDIA_PROVIDER',scope,secretFingerprint:fingerprint,nonce:`${scope}-${crypto.randomUUID()}`,issuedAt:new Date().toISOString(),expiresAt:new Date(Date.now()+30_000).toISOString()}),brandSnapshot:{id:requiredEnv('MEDIA_BRAND_SNAPSHOT_ID'),digest:requiredEnv('MEDIA_BRAND_SNAPSHOT_DIGEST'),state:'APPROVED',approvedAt:snapshotAt,expiresAt:null},knowledgeSnapshot:{id:requiredEnv('MEDIA_KNOWLEDGE_SNAPSHOT_ID'),digest:requiredEnv('MEDIA_KNOWLEDGE_SNAPSHOT_DIGEST'),state:'APPROVED',approvedAt:snapshotAt,expiresAt:null}});
}
...
void loop();
for(const signal of ['SIGTERM','SIGINT'] as const)process.once(signal,()=>{stopping=true;server.close(()=>{void Promise.allSettled([runtime.close(),goals.close(),artifacts.close(),mediaRepository?.close()]).finally(()=>{process.exitCode=0;});});});
...
async function readiness(){
  ...
  const verified=agentTeams.identityEvidence?.verified===true;return {service:'mission-worker',state,reasonCode,controlPlane:{state:postgresql?'READY':'UNREACHABLE',source:'POSTGRESQL'},worker:{state:worker===undefined?'NOT_CONFIGURED':heartbeatFresh?'READY':'STARTING',workerId,heartbeatAt,lastTick},mediaWorker:{mode:mediaMode,state:mediaWorker===undefined?'NOT_CONFIGURED':'READY',heartbeatAt:mediaHeartbeatAt,lastTick:mediaLastTick,purpose:'MEDIA_PROVIDER',providerEvidence:mediaMode==='REAL_PROVIDER'},gateway,agentTeams:{state:agentTeams.state,memberCount:agentTeams.memberCount,version:agentTeams.runtimeVersion,sourceCommit:verified?agentTeams.identityEvidence!.expected.sourceCommit:'UNVERIFIED',sourceTarSha256:agentTeams.runtimeDigest,teamProfileVersion:agentTeams.teamProfileVersion,teamProfileDigest:agentTeams.teamProfileDigest,imageDigests:agentTeams.identityEvidence?.actual.images??[],pinnedIdentityVerified:verified,identityEvidence:agentTeams.identityEvidence},authority:{mode:'LOCAL_DOCKER_HOST_SUPERVISOR',dockerSocketMounted:false,fixedCommandSurface:true,webDockerAuthority:false,apiDockerAuthority:false,gatewayDockerAuthority:false},externalActionAllowed:false,secretPresentInResponse:false};
}
