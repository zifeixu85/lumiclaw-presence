...
try{
  ...
  docker(['exec','-T','-e','DATABASE_URL=postgres://postgres@postgres:5432/lumiclaw_sdd008_empty_down','api','npm','--workspace','@lumiclaw/db','run','migrate:down']);
  ...
  docker(['exec','-T','-e','DATABASE_URL=postgres://postgres@postgres:5432/lumiclaw_sdd008_legacy','api','npm','--workspace','@lumiclaw/db','run','migrate:down']);
  ...
  const legacyDown=dockerExpectedFailure(['exec','-T','-e','DATABASE_URL=postgres://postgres@postgres:5432/lumiclaw_sdd008_legacy','api','npm','--workspace','@lumiclaw/db','run','migrate:down']);
  ...
  const mainDown=dockerExpectedFailure(['exec','-T','api','npm','--workspace','@lumiclaw/db','run','migrate:down']);
  if(mainDown.status===0||!mainDown.output.includes('SDD008_DOWN_BLOCKED_DATA_EXPORT_AND_OWNER_DECISION_REQUIRED')||pg("select to_regclass('public.knowledge_snapshots') is not null")!=='t')throw new Error('SDD008_MAIN_POPULATED_DOWN_NOT_BLOCKED');checks.populatedMainDownPreservesSchemaAndData=true;
