...
try {
  ...
  // M3-01: action_grants is expected. Only M3-02+ tables remain forbidden.
  const forbiddenTables = Number((await pool.query("select count(*)::int as count from information_schema.tables where table_schema='public' and table_name in ('connectors','action_outbox')")).rows[0].count); await pool.end();
