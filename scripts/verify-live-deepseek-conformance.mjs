...
import {LIVE_SUBMISSION_IMPORT_OUTCOMES} from './live-submission-import-outcome.mjs';
...
async function verifySubmissionImportDiagnostics() {
  const organizationId = '019fcc41-dd89-70c1-ae55-c8e45b4aeb3f'; const missionId = '019fcc41-ddba-7897-a271-d0eda0c9a7fd'; const campaignDigest = 'f'.repeat(64);
  const bootstrap = 'public-safe-submission-import-evidence-bootstrap-0001'; const dummySecret = 'dummy-secret-submission-import-evidence-0001'; const dummyTicket = 'dummy-ticket-submission-import-evidence-0001'; const rawResponse = 'raw-response-submission-import-evidence-0001';
  const diagnosticRoot = await mkdtemp(path.join(tmpdir(), 'lumiclaw-submission-import-evidence.')); const receipts = [];
  try {
    for (const submissionImportOutcomeCode of LIVE_SUBMISSION_IMPORT_OUTCOMES) {
      const evidencePath = path.join(diagnosticRoot, `${submissionImportOutcomeCode}.json`);
      const result = spawnSync(process.execPath, ['scripts/verify-agentteams-real-environment.mjs', `--live-submission-import-outcome-diagnostic-conformance=${submissionImportOutcomeCode}`], {cwd: root, input: JSON.stringify({organizationId, missionId, campaignDigest, bootstrap}), encoding: 'utf8', timeout: 15_000, stdio: ['pipe', 'pipe', 'pipe'], env: {...process.env, LUMICLAW_LIVE_FAILURE_EVIDENCE_PATH: evidencePath, DUMMY_SECRET_MARKER: dummySecret, DUMMY_TICKET_MARKER: dummyTicket, DUMMY_RAW_RESPONSE_MARKER: rawResponse}});
      const output = `${result.stdout}${result.stderr}`;
      if (result.status === 0 || result.stdout !== '' || [bootstrap, dummySecret, dummyTicket, rawResponse].some((marker) => output.includes(marker)) || /x-lumiclaw-runner-bootstrap|x-lumiclaw-runtime-ticket|authorization|\bbearer\b|responseId|details|Traceback|Exception/iu.test(output)) throw new Error('LIVE_SUBMISSION_IMPORT_DISCLOSURE_OR_STATUS_INVALID');
      const envelope = parseLiveFailureEnvelope(result.stderr, {missionId}); const receipt = JSON.parse(await readFile(evidencePath, 'utf8'));
      if (envelope.stage !== 'TASK_PROTOCOL' || envelope.code !== 'LIVE_TASK_PROTOCOL_FAILED' || envelope.providerOutcomeCode !== null || envelope.taskProtocolOutcomeCode !== 'LIVE_TASK_SUBMISSION_IMPORT_FAILED' || envelope.submissionImportOutcomeCode !== submissionImportOutcomeCode || !isLiveFailureReceipt(receipt, {organizationId, missionId, campaignDigest, stage: 'TASK_PROTOCOL', code: 'LIVE_TASK_PROTOCOL_FAILED', taskProtocolOutcomeCode: 'LIVE_TASK_SUBMISSION_IMPORT_FAILED', submissionImportOutcomeCode})) throw new Error('LIVE_SUBMISSION_IMPORT_RECEIPT_INVALID');
      receipts.push({submissionImportOutcomeCode, failedTaskBound: receipt.failedTaskId === 'public-safe-task-protocol-task'});
    }
    return {status: 'PASS', actualNestedChildProcess: true, cases: receipts.length, outcomes: receipts, arbitraryApiCodeMapsTo: 'LIVE_SUBMISSION_IMPORT_UNCLASSIFIED', arbitraryExceptionTextForwarded: false, rawApiOrModelOutputForwarded: false, bootstrapTicketHeaderResponseIdFinding: false};
  } finally { await rm(diagnosticRoot, {recursive: true, force: true}); }
}
...
try {
  ...
  const submissionImportDiagnostics = await verifySubmissionImportDiagnostics();
