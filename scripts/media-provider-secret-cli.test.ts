...
describe('terminal-only isolated media Provider Secret gate',()=>{
  it('writes 0700/0600, emits only a purpose-bound fingerprint, and never returns the key',async()=>{const cli=await isolated();const key='public-safe-media-provider-test-value';const value=await cli.configureMediaSecret(async()=>key);expect(value).toMatchObject({state:'READY',configured:true,profileMaturity:'REAL_PROVIDER_CANARY_READY',fingerprint:expect.stringMatching(/^[a-f0-9]{16}$/u)});expect(JSON.stringify(value)).not.toContain(key);expect((await stat(process.env.LUMICLAW_MEDIA_SECRET_ROOT!)).mode&0o777).toBe(0o700);expect((await stat(path.join(process.env.LUMICLAW_MEDIA_SECRET_ROOT!,'media-provider-key'))).mode&0o777).toBe(0o600);expect(await readFile(path.join(process.env.LUMICLAW_MEDIA_SECRET_ROOT!,'media-provider-key'),'utf8')).toBe(`${key}\n`);});
