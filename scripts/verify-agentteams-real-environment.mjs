...
async function spawnLiveRunner(serialized, parsed, options = {}) {
  const transportOnly = options.transportConformance === true; const diagnosticStage = options.diagnosticStage; const diagnosticProviderOutcome = options.diagnosticProviderOutcome; const diagnosticTaskProtocolOutcome = options.diagnosticTaskProtocolOutcome; const diagnosticSubmissionImportOutcome = options.diagnosticSubmissionImportOutcome;
  const diagnostic = diagnosticStage !== undefined || diagnosticProviderOutcome !== undefined || diagnosticTaskProtocolOutcome !== undefined || diagnosticSubmissionImportOutcome !== undefined;
  const result = spawnSync(process.execPath, ['scripts/run-live-deepseek-uat.mjs', ...(transportOnly ? ['--transport-conformance'] : []), ...(diagnosticStage === undefined ? [] : [`--diagnostic-stage-conformance=${diagnosticStage}`]), ...(diagnosticProviderOutcome === undefined ? [] : [`--provider-outcome-diagnostic-conformance=${diagnosticProviderOutcome}`]), ...(diagnosticTaskProtocolOutcome === undefined ? [] : [`--task-protocol-outcome-diagnostic-conformance=${diagnosticTaskProtocolOutcome}`]), ...(diagnosticSubmissionImportOutcome === undefined ? [] : [`--submission-import-outcome-diagnostic-conformance=${diagnosticSubmissionImportOutcome}`])], {
...
} else if (diagnosticStageConformance !== undefined || diagnosticProviderOutcomeConformance !== undefined || diagnosticTaskProtocolOutcomeConformance !== undefined || diagnosticSubmissionImportOutcomeConformance !== undefined) {
  try {
    ...
    const input = await readCanonicalLiveRunnerInput();
    await spawnLiveRunner(input.serialized, input.parsed, {diagnosticStage: diagnosticStageConformance, diagnosticProviderOutcome: diagnosticProviderOutcomeConformance, diagnosticTaskProtocolOutcome: diagnosticTaskProtocolOutcomeConformance, diagnosticSubmissionImportOutcome: diagnosticSubmissionImportOutcomeConformance});
