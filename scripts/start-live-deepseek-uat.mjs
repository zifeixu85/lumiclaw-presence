...
const allowedChildFailureCodes = new Set(['LIVE_UAT_TRANSPORT_INVALID', 'LIVE_UAT_TRANSPORT_FAILED', 'LIVE_UAT_TRANSPORT_DISCLOSURE_BLOCKED', 'LIVE_UAT_RUNNER_RECEIPT_INVALID', 'LIVE_DEEPSEEK_UAT_RUNNER_FAILED', 'LIVE_AGENTTEAMS_ENVIRONMENT_FAILED', 'LIVE_UAT_CLEANUP_FAILED']);
...
try {
  ...
  if (result.status !== 0) throw stableError(parseStableChildFailure(stderr));
  let childReceipt;
  ...
  failureCode = typeof error?.code === 'string' && allowedChildFailureCodes.has(error.code) ? error.code : 'LIVE_UAT_LAUNCHER_FAILED';
} finally {
  if (composeStarted && secretRoot !== undefined) {
    ...
    try { compose(['down', '--volumes', '--remove-orphans'], environment, 'ignore'); } catch { failureCode = 'LIVE_UAT_CLEANUP_FAILED'; }
  }
  if (secretRoot !== undefined) {
    try { await rm(secretRoot, {recursive: true, force: true}); } catch { failureCode = 'LIVE_UAT_CLEANUP_FAILED'; }
  }
}
if (failureCode !== undefined) { writeSync(2, `${JSON.stringify({status: 'FAIL', code: failureCode})}\n`); process.exitCode = 1; }
