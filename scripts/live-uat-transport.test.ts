...
describe('Live UAT nested child-process stdin transport', () => {
  ...
  it('returns only a stable redacted code when valid transport reaches an operational failure', () => {
    ...
    expectNoDisclosure(result, bootstrap, secretMarker);
    expect(result.stdout).toBe('');
    expect(JSON.parse(result.stderr)).toEqual({status: 'FAIL', code: 'LIVE_MISSION_OPEN_FAILED', stage: 'MISSION_OPEN', providerOutcomeCode: null, taskProtocolOutcomeCode: null, submissionImportOutcomeCode: null, taskProtocolStatus: {planStatus: null, taskStatus: null}, missionId, evidence: '.evidence/sdd-002/deepseek-live-failure.json', secretPresent: false, liveProviderVerified: false});
  });
});
