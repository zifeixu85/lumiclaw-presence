...
export function checkAcceptanceReport(markdown) {
  const missingHeadings = requiredHeadings.filter((heading) => !markdown.includes(heading));
  const missingCriteria = Array.from({length: 18}, (_, index) => `AC-${String(index + 1).padStart(2, '0')}`)
...
if (process.argv[1] !== undefined && import.meta.url === new URL(`file://${path.resolve(process.argv[1])}`).href) {
  ...
  const result = checkAcceptanceReport(await readFile(target, 'utf8'));
  ...
  console.info(JSON.stringify({status: 'PASS', report: path.relative(process.cwd(), target), criteria: 18}));
}
