# SDD-001 Dependency Register

> Scope: local Campaign walking skeleton plus the retained delivery foundation. This register is not a production security or legal-compliance claim.
...
| Next.js / React | `16.2.12` / `19.2.8` | MIT | INTEGRATE | `apps/web` only | M1 persistent workspace and native-like previews; no production exposure claim. |
...
| Fastify | `5.11.0` | MIT | INTEGRATE | Campaign REST/OpenAPI control boundary | Replaceable HTTP boundary; no external platform endpoint. |
| pg / Kysely / node-pg-migrate | `8.22.0` / `0.29.4` / `9.0.0` | MIT | INTEGRATE | PostgreSQL Campaign, revision, idempotency and schedule persistence | Only the project-scoped local test database is targeted. |
| Ajv | `8.20.0` | MIT | INTEGRATE | Domain, Campaign and AgentTeams contract validation | No runtime orchestration behavior is copied. |
...
- M1 accepts these only as a local, visibly non-live engineering baseline: no customer input, uploaded CSS/image, production deployment or external account is allowed. A production release remains blocked until an upstream Next 16 fix or a separately reviewed patch is available.
...
- Full transitive inventory: `.evidence/sdd-001/license-inventory.json`
- CycloneDX SBOM: `.evidence/sdd-001/sbom.cdx.json`
