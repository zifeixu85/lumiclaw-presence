...
| CL-29 | CR2 Fix 8 authorizes `TASK_ACK` and `MODEL_GENERATE` by the exact tuple of action, Task kind and Mission phase. Initial work is `RUNNING`-only, Founder correction is `REVISION_REQUIRED`-only, and independent re-audit is `AUDIT_BLOCKED`-only. |
| CL-30 | Later-phase authorization never relaxes Task state, role, attempt, input projection, ticket expiry/single-use or runtime identity bindings. The stable `LIVE_RUNTIME_ACTION_NOT_READY` response is sufficient and no raw ticket-policy detail crosses the broker boundary. |
