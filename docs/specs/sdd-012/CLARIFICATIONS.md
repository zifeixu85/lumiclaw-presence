...
| CL-31 | 当前 v4 Audit 只实现 public-safe controlled engineering fixture：route closed schema 禁止客户端提交 `auditorIdentityId` 或 runtime receipt，服务端固定 A5 identity，并持久化 `evidenceMaturity=CONTROLLED_FIXTURE`、`agentTeamsExecuted=false`、`authoritativeForOperations=false`。该 PASS 不能创建 Owner `APPROVE` 或 ManualPublishPackage；真实运营路径保持 `BLOCKED/PLANNED`，等待 SDD-007 exact accepted A5 Auditor task/output receipt。未来接入时 repository 必须重读 owner/run/job/task/attempt、A5 role、Skill locks、input revision/media/snapshot digests 与 accepted output schema/canonical digest，不能信任请求字段。 |

无未决产品设计歧义；真实 A5 runtime Audit authority 是已明确的 `BLOCKED/PLANNED` 依赖，不属于本轮伪造字段可替代的实现。decoder/normalizer/compositor/archive/font/template/Logo 资产仍需完成 exact version/digest/license/NOTICE/SBOM/security 决定；选择只能满足本规格，不能降低 AC。
