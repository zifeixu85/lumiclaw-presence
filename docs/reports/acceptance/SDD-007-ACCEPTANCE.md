...
>
> Commit / Draft PR：由本报告所在交付提交与最终 `STATUS_HANDOFF` 固定
>
...
5. 在第三个终端执行 `npm run runtime:secret:status`、`curl --fail http://127.0.0.1:4100/api/v1/runtime/readiness` 和 `curl --fail http://127.0.0.1:4401/health`。controlled fake 预期 `DEGRADED`；real provider 只有四路全部通过才可 `READY`。错误 digest/profile、missing heartbeat、`INCOMPATIBLE/UNREACHABLE` 都是失败。
