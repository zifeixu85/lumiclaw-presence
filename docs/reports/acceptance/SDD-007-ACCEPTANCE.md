...
>
> Runtime evidence source HEAD：`ce4c335111fddd0197b4e4f34cd83c28f6b2d07b`（内外层 real evidence 均绑定该 clean committed source HEAD；此前 `0bc69f639ecfc3269622c3ff86379f70d4eef957` 证据已被本轮 final-P1 证据取代）
>
...
- controlled-real verifier 只走 production `PersistentMissionWorker.tick`；五个领域成员必须各自从 exact-role Worker 容器发出 Gateway HTTP，Leader provider call 为 0。Gateway 将容器内 `socket.gethostname()` 与 actor header 同 signed request body、`docker inspect .Config.Hostname` 和 pinned binding 核对，只持久 public-safe hostname digest；不依赖 Docker Desktop 可能代理为 loopback 的 `remoteAddress`。最终 observation 按 DAG `assigned_to` 并受 binding role allowlist 约束，从各自 exact Worker store 聚合 6 个 submitted digest，不以 Leader store 代替成员结果。门禁在 submit→stage 与 finalize→completion confirmation 之间各注入一次 crash，并对 AgentTeams v1.2.0 `_replace_task_status` 的 missing/reassigned silent no-op 做真实 upstream-compatible 负测；三次新 worker/restart 后通过 exact submission observation、completion outbox 与写后读回收敛。
- migration `000014` 建立 run/job/attempt/binding/event、heartbeat、ticket、staging batch/item 等权威表。fresh PostgreSQL 覆盖 acquire 后 fenced heartbeat、双 worker lease CAS、ACK/no-intent 单赢家恢复、过期/retry/stale lease ticket fencing、并发 one-use、durable submission intent、跨域 staging 原子提交、completion outbox，以及 confirmation 前不释放后继/终态的 crash recovery。completion acquire/confirm/advance/reconcile 均保持 terminal monotonic；cancel 持 run row lock，显式写 `RUNTIME_COMPLETION_ABANDONED_RUN_CANCELLED`、撤销 completion lease，旧 worker 不能 confirm 或复活 `CANCELLED` run。
...
| Recovery | acquire 后立即 fenced heartbeat；外部操作有 hard timeout 与 TERM→KILL。ACK 后、intent 前 crash 由同一 attempt 的新 lease 单赢家领取并返回 `envelope:null`，从幂等 ACK 继续。Model 输出在 AgentTeams submit 前写入 immutable submission intent；submit→stage crash 后 observe exact runtime payload/digest，缺失时只重放同一 payload，不再调用 provider。`STAGED` 仅走专用 recovery。finalize 原子提交产品写与 runtime ACCEPTED；durable completion outbox 随后以完整 TaskContract 调用 AgentTeams typed internal completion，并写后读回 exact taskId、frozen assigned role 与 `completed`；missing/reassigned/unchanged 均 fail closed。只有 PG exact confirmation 事务提交后才释放依赖或写 run 终态；cancel 会 abandon pending completion，terminal run 不可逆。 |
...
- completion outbox 的 acquire query 同时锁定 run/batch/job/attempt 并排除全部 terminal run；confirm 先锁 run 并拒绝 terminal state，advance 与 reconcile 也有 terminal guard。API/fresh-PG 对抗覆盖 cancel 对已领取 completion 的显式 abandon、旧 lease confirm 拒绝、双 worker/restart 不可再次领取，以及 reconcile 对 `CANCELLED` 原样返回；accepted output 保留为审计证据，但 downstream 全部取消且 run 永不复活。
- AgentTeams v1.2.0 `_replace_task_status` 对 missing task 可能静默 no-op。driver 因此接收完整 TaskContract，只有 target 当前 assigned role 与 frozen binding 一致才写，随后重新读取 exact task，并核对 taskId、`assigned_to` 与 `status=completed`。missing/reassigned/unchanged 的 unit 负测全部 fail closed；clean-source 真实 verifier 另对 missing/reassigned upstream no-op 负测，失败时 PG 不 confirm、downstream 不释放。
...
| Evidence | 内容 | SHA-256 |
|---|---|---|
| `.evidence/sdd-007/postgres.json` | fresh PG、lease heartbeat/CAS、ACK/no-intent recovery、ticket fencing、durable intent、staged atomic recovery、cancelled-run completion terminal monotonic、ETag/idempotency | `94abc7754ec88b6965580db4bb30f001b49c7b92d30278534317cbc8ebdcc734` |
| `.evidence/sdd-007/compose.json` | controlled-fake missing-heartbeat `UNREACHABLE`、Web/API/CLI parity、Gateway/API/PostgreSQL restart、Secret/Docker scope、exact cleanup | `c2ea9c3cf04c9428feeb42fe8a55d4bd393dd3d5a7eaab590b5073feb4b839bc` |
...
| `.evidence/sdd-007/agentteams-persistent-driver.json` | clean source HEAD、production worker path、Leader 0 call、exact Worker challenge、2 个 crash barrier、missing/reassigned completion no-op fail closed | `e2ba161c71f53780a261c6233575a22aeb5bb6e2b0853f1d5d00f06752fc6f12` |
| `.evidence/sdd-002/agentteams-real-runtime.json` | 同一 clean source HEAD、official installer、source/license/image identity、真实 runtime lifecycle 与 exact cleanup | `989f7c263b70d5de9df4d6c5aa11e3ce8a42c1509249293f82094832280f3f86` |
...
| `npx vitest run apps/web/src/components/production-workspace.test.ts apps/api/src/runtime-readiness.test.ts apps/mission-worker/src/readiness.test.ts apps/mission-worker/src/worker.test.ts apps/mission-worker/src/output-authority.test.ts packages/domain/src/runtime-output-schemas.test.ts packages/runtime-agentteams/src/persistent-driver.test.ts scripts/verify-sdd007-agentteams-driver.test.ts --configLoader=runner` | `PASS`，8 files / 35 tests；真实无 campaign AI Team、missing/stale heartbeat、DB-down health、立即 heartbeat/lease fencing、hard timeout cleanup、ACK/no-intent 单次 provider、durable submit/completion recovery、cancel 后 completion confirm fail closed、Leader 0 call、trusted audit clock、assigned-worker observation、missing/reassigned/unchanged completion 与 real-gate source/crash 合同。 |
...
| `npm run verify:sdd007:agentteams-real` | `PASS`，clean committed source HEAD `ce4c335111fddd0197b4e4f34cd83c28f6b2d07b`；内外层 evidence HEAD/branch 一致，actual pinned v1.2.0 identity verified；production worker path 6 ACK/Submit/check、6 assigned-worker result digests、5 inspect-matched exact-role Worker-origin Gateway calls、Leader 0 call；submit-stage 与 finalize-complete 两个 crash barrier、missing/reassigned completion no-op 负测、3 次 worker restart、completion recovery count=2、cleanup PASS。controlled fake=true、real DeepSeek=false、external action=0。 |
...
| `npm test` | `PASS`，63 passed / 4 skipped files；505 passed / 4 skipped tests。 |
...
| AC-07 | `PASS` | external submit 前持久 immutable output intent；submit→stage crash 通过 exact observe 恢复且 provider 不重跑。staged candidate 对产品 authority/head/idempotency 不可见，产品写与 runtime ACCEPTED 同事务；accepted 后 completion outbox 可跨 crash/timeout 幂等确认；AgentTeams complete 写后读回 exact task/role/status，missing/reassigned/unchanged fail closed，PG confirmation 前不释放 downstream 或 run 成功终态。 |
...
| AC-11 | `PASS` | cancel/retry 要求 Idempotency-Key + If-Match；并发/replay/stale ETag 负测，412 body code 与 current ETag 稳定。cancel 锁 run、abandon 已领取 pending completion 并撤销 lease；acquire/confirm/advance/reconcile 均不能复活 terminal run。 |
...
10. provider 已返回成功、但 immutable submission intent 尚未提交 PostgreSQL 的极短 crash window，恢复 worker 可能再次调用 provider；当前 exactly-once 保证从 intent commit 开始，不把该窗口误写为 provider exactly-once。后续可用 provider request idempotency 或先写 request intent 缩小边界。
11. GitHub `real-agentteams-arm64` 是 `workflow_dispatch` + self-hosted macOS ARM64 的手动证据 job，普通 PR CI 可以显示 skipped；这不等于远端 real gate PASS。本轮 clean committed source HEAD 已在本地实际执行 `npm run verify:sdd007:agentteams-real` 并 PASS，但远端手动 ARM64 job 仍未声明运行。
...
| Runtime evidence source HEAD | `ce4c335111fddd0197b4e4f34cd83c28f6b2d07b`（内外层 PASS evidence 同 HEAD/branch） |
