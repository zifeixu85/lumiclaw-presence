...
| `05-full-content-review-drawer.png` | `43a2123220e1ceff4faa318c3805523280b5bb758311bd1503dcdcbf8d5f3144` |
...
| `browser-verification.json` | `3284c71d65ec44f898de1af6b021aaa5c8dd986ed3e19d9e0eeea5ebcaec69c8` |
| `.evidence/sdd-006/compose-verification.json` | `8270fe4123296cdd69ccfc1abdef683c6c1dfa540b81f185393dccf0e13382f4` |
...
| `npm run verify:sdd006:compose` | `PASS`，12 Compose/PG checks + 33 browser checks + 14 screenshots；cleanup PASS。 |
...
5. Campaign → 完整内容：预期阅读全文、当前 Agent 与 trace；缺少 Audit receipt 时批准 disabled；Tab 留在抽屉，Esc 关闭并恢复焦点。完成 Onboarding 后依次调用 `materials-path`、`example`、`context`、重复 `complete` 与上传新 MD/TXT，均应返回 `409 LOCAL_ONBOARDING_ALREADY_COMPLETED`；随后删除已绑定资料仍应返回 `409 LOCAL_MATERIAL_BOUND_TO_CAMPAIGN`。重开后 profile/session、材料与 Campaign digest/EvidenceRef 必须完全不变。
...
- Objective：修复 PR #7 第一/二轮 P1 以及第三轮“完成态 Onboarding 可重入并绕过 Campaign 证据删除保护”，并在同一分支提供可重复工程证据。
...
`PENDING`。Coordinator 需在最终 remote full SHA 上复核第一/二轮 P1、第三轮完成态重入对抗链、changed files、CI、AC/证据 digest 与 Owner UAT，再决定 canonical state。
