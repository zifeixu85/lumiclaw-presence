...
> Final Head / Commits：初始 closeout `c26cb9e24c062bf6793f1f4a191f4548dd46a63f`；Coordinator 精确计费修正 `4dc33de589dd3509c749490dc3662e561f0e9ac9`；CR2 首次工程 Head `c931b77ff723e3d9a098a8a088e8a6e3ffe7cb79`；stdin transport 修正后的 final clean Head 由结构化 handoff 与 final source-package manifest 精确记录
...
| Runtime Adapter | `packages/runtime-agentteams/src/shadow-adapter.ts`、`scripts/verify-agentteams-real-environment.mjs`、`scripts/verify-agentteams-real-runtime.mjs`、`scripts/run-live-deepseek-uat.mjs`、`scripts/live-uat-transport.mjs` | 官方 installer 自举、真实 Project/DAG/ACK/Submit、authenticated import、八 Task 三阶段因果状态、RoleContext projection、restart、digest/schema/duplicate quarantine、同一 Campaign/Mission/Project binding 与精确清理；Live 仅绑定单 Mission/Campaign/runtime digest，七个领域 Task receipt，Leader 无模型；strict JSON stdin 与 nested child-process non-disclosure；`.evidence/sdd-002/agentteams-real-runtime.json`，真实 Canary待 Coordinator 生成 `.evidence/sdd-002/deepseek-live-canary.json`。 |
...
| Security / License | `compose.live-deepseek-uat.yml`、`start-live-deepseek-uat.mjs`、`live-uat-transport.test.ts`、`check-compose-policy.mjs`、Storybook client-bundle safety、`secret-scan.mjs`、dependency inventory、CycloneDX SBOM | 交互式无回显→0600 临时文件→Compose read-only secret；四字段 JSON 只经 captured stdin pipe，raw child output 不继承；inspect/env/log/bundle/source 无值泄漏；无 Docker socket；public-safe packaging、license/provenance、npm audit、SBOM 与 final manifest。 |
...
| 检查 | 命令或协议 | 期望 | 实际 | 结果 |
|---|---|---|---|---|
| Unit / Schema / Permission | `npm test` | 正反合同、六角色、权限、runtime/provider/media/audit/review/UI 全通过 | 23 个 test files、144 个 tests 全通过；新增 Live state、ticket scope/reuse/expiry、Leader no-model、Auditor separation、no-key/no-fallback、redaction 与真实 nested stdin transport 合同 | `PASS` |
...
| Live 合同 / Secret 防泄漏 | `npm run verify:live-conformance` | 无 Key/错 scope/复用/过期/Leader 调用模型 fail closed；Secret 不在普通 env/inspect/log/stdin child output；仅固定 `/run/secrets/...` read-only mount；无 Docker socket/Mock fallback | 3 files / 21 定向 tests `PASS`；5 个实际 child-process cases 覆盖完整/partial/malformed/extra-field/operational failure，raw bootstrap/dummy secret/header finding 0；隔离 project `lumiclaw-sdd002-live-conformance` 的 `secretInEnvironment=false`、两个固定 secret mount、`dockerSocket=false`；Compose project 与包含 0600 Secret 文件的私有临时目录精确清理；证据 `live-deepseek-conformance.json` | `PASS` |
...
| Build / Storybook / Static | `npm run verify` | lint/type/test/messages/status/report/secret/compose/profile/license/SBOM/build/Storybook | lint 零 warning；23 files/144 tests；99 双语 keys；39 status modules；依赖/License/SBOM、Next production build 与 Storybook browser-safety 全通过 | `PASS` |
...
- **操作步骤：**
  ...
  3. 从页面复制三个公开标识：Organization ID、Live Mission ID、Campaign digest，依次粘回终端提示；不要粘贴 Key。启动器会在内存中生成严格四字段 JSON 并通过 captured stdin pipe 交给 Runner，不应显示 runtime bootstrap、ticket 或 request header。Runner 应自举官方 AgentTeams v1.2.0 精确六成员并只领取该 Mission；失败只显示 `{"status":"FAIL","code":"<stable-code>"}`。
