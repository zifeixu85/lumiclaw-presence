...
Coordinator 在 Fix 2 clean Head `356a9cac79d49cbda05b5fad8855c93363159183` 的第三次真实 Canary 中确认所有 pre-dispatch 进度、Provider broker request started 与清理均通过，但首个领域 Task 仍以顶层 `PROVIDER_REQUEST / LIVE_PROVIDER_REQUEST_FAILED` 失败，model receipt 0。宿主收据 SHA-256 为 `2396c679f12bbbf2cc78c1c2c81f211f226c67cd8cd797253162f2db10746248`；它不含 Secret/raw response，但旧 Worker 使用 `urllib.request.urlopen`，非 2xx 会在解析 API 脱敏错误 envelope 前抛出 `HTTPError`，所以 cleanup 后不能区分 HTTP、Provider identity/finish/usage/JSON/schema/semantic 或 broker failure。Coordinator 随后用隐藏 TTY 独立确认 Owner credential、官方 `/models`、`deepseek-v4-flash`、`json_object`、response identity 与 usage 基本合同均为 HTTP 200；这些独立 probe 不等于产品首个 Task 成功，本次仍为 `LIVE_PROVIDER_VERIFIED=false`。

Fix 3 在不接触 Owner Key 的前提下收口了诊断与产品合同。API 只返回既有 stable provider/model/semantic code；Runner 在 Worker 非零退出后重新读取精确 Mission，以 failed Task、Mission failure 和 `ModelCallSnapshot.error.code` 三者一致性导出 `providerOutcomeCode`，任一缺失、冲突、额外或非 allowlist 状态统一关闭为 `LIVE_PROVIDER_BROKER_FAILED`。顶层 stage/code 保持不变，宿主收据和两层 child envelope 只增加一个 nullable allowlisted 字段，不透传 body、prompt、model content、response ID、headers、ticket、bootstrap 或任意异常文本。源码检查同时发现一个具体产品缺陷：Gateway 会本地验证 `outputSchema`，但此前没有把 schema 发送给 DeepSeek，system prompt 所称“supplied role schema”实际不存在；现在 exact schema 同时进入 user message 与 input digest。公开安全首任务 fixture 证明错误 shape 以 `MODEL_SCHEMA_INVALID` 拒绝，精确 `{frozen:true, assessment:<非空>}` 接受。由于第三次 Canary 已丢失 task-level code，本报告只把该缺陷列为已验证并修复的合同缺陷，不声称它必然就是第三次失败的历史根因；下一次 Coordinator 重试会在一次失败内返回精确 allowlisted outcome。
...
- Live failure diagnostics：`MISSION_OPEN / RUNTIME_IDENTITY / TOPOLOGY / PROJECT_CREATE / DAG_PLAN / MEMBER_BINDING / PROJECT_DISPATCH / TASK_PROTOCOL / PROVIDER_REQUEST / FINALIZE / AGENTTEAMS_PROVISION / CLEANUP` 各有唯一 stable code；`.evidence/sdd-002/deepseek-live-failure.json` 在清理前原子持久化、清理后只更新 allowlisted cleanup 状态，并从 public source ZIP/run manifest 排除。
...
| Runtime Adapter | `packages/runtime-agentteams/src/shadow-adapter.ts`、`scripts/verify-agentteams-real-environment.mjs`、`scripts/verify-agentteams-real-runtime.mjs`、`scripts/run-live-deepseek-uat.mjs`、`scripts/live-uat-transport.mjs`、`scripts/live-uat-diagnostics.mjs`、`scripts/live-provider-outcome.test.ts` | 官方 installer 自举、真实 Project/DAG/ACK/Submit、authenticated import、八 Task 三阶段因果状态、RoleContext projection、restart、digest/schema/duplicate quarantine、同一 Campaign/Mission/Project binding 与精确清理；Live 仅绑定单 Mission/Campaign/runtime digest，七个领域 Task receipt，Leader 无模型；strict JSON stdin、12-stage allowlist、task-bound provider outcome、host failure receipt 与 nested child-process non-disclosure；`.evidence/sdd-002/agentteams-real-runtime.json`、`live-agentteams-no-secret-diagnostic.json`，真实 Canary待 Coordinator 生成 `deepseek-live-canary.json`。 |
...
| Provider / Media | `packages/governed-shadow/src/model-provider.ts`、`packages/governed-shadow/src/media-provider.ts`、`infra/providers/` | DeepSeek official gateway/mock transport conformance；exact role output schema 进入 Provider message 和 input digest；flash/pro cache hit/miss/output 精确计费与 usage consistency fail-closed；Live receipt 记录 provider/model/usage/cost/latency/schema/digest/error 且 redacted；EvoLink no-key boundary；rights/cost/unreviewed content-addressed media；`.evidence/sdd-002/provider-conformance.json`。 |
...
| Security / License | `compose.live-deepseek-uat.yml`、`start-live-deepseek-uat.mjs`、`live-uat-transport.test.ts`、`live-uat-diagnostics.test.ts`、`check-compose-policy.mjs`、Storybook client-bundle safety、`secret-scan.mjs`、dependency inventory、CycloneDX SBOM | 交互式无回显→0600 临时文件→Compose read-only secret；四字段 JSON 只经 captured stdin pipe，raw child output 不继承；12 stage/code/envelope/receipt exact schema，dummy bootstrap/ticket/Authorization/Bearer/raw response finding 0；inspect/env/log/bundle/source 无值泄漏；无 Docker socket；local failure receipt不进入 public ZIP/manifest；license/provenance、npm audit、SBOM 与 final manifest。 |
...
| 检查 | 命令或协议 | 期望 | 实际 | 结果 |
|---|---|---|---|---|
| Unit / Schema / Permission | `npm test` | 正反合同、六角色、权限、runtime/provider/media/audit/review/UI 全通过 | 25 个 test files、229 个 tests 全通过；新增 Live state、ticket scope/reuse/expiry、Leader no-model、Auditor separation、no-key/no-fallback、redaction、12-stage exact receipt、20-outcome task-bound provider diagnostics 与真实 nested child transport 合同 | `PASS` |
...
| Provider / Media | `npm run verify:providers` | structured/429/5xx/4xx/timeout/schema/no-switch/redaction/exact cost/media rights/no approval | `status=PASS`；DeepSeek/EvoLink provider conformance 外部 Canary `NOT_RUN_NO_KEY`；Mock `MOCK_CONFORMANCE`；exact role schema 进入 message/input digest；flash `(20×0.0028 + 80×0.14 + 50×0.28)/1e6 = 0.000025256 USD`，pro `(20×0.003625 + 80×0.435 + 50×0.87)/1e6 = 0.0000783725 USD`；细分和矛盾或只出现 hit/miss 一项均拒绝；8 个合法 Role projection通过、15 个越权/篡改输入拒绝；Secret absent | `PASS` |
| Live 合同 / Secret 防泄漏 | `npm run verify:live-conformance` | 无 Key/错 scope/复用/过期/Leader 调用模型 fail closed；Secret 不在普通 env/inspect/log/stdin child output；仅固定 `/run/secrets/...` read-only mount；无 Docker socket/Mock fallback | 6 files / 126 定向 tests `PASS`；5 个 stdin cases + 12 个实际 nested stage child cases + 20 个 provider-outcome nested child cases；raw bootstrap/dummy secret/ticket/header/raw response/response ID finding 0；隔离 project `lumiclaw-sdd002-live-conformance` 的 `secretInEnvironment=false`、两个固定 secret mount、`dockerSocket=false`；Compose project 与 0600 临时目录精确清理；证据 `live-deepseek-conformance.json` | `PASS` |
...
| Build / Storybook / Static | `npm run verify` | lint/type/test/messages/status/report/secret/compose/profile/license/SBOM/build/Storybook | lint 零 warning；25 files/229 tests；99 双语 keys；39 status modules；依赖/License/SBOM、Next production build 与 Storybook browser-safety 全通过 | `PASS` |
...
| Source ZIP / Evidence | `npm run evidence:package-source && npm run evidence:manifest` | clean committed Head；文件/bytes/SHA-256/Secret/path/CRC/content-to-Git scan；必要 evidence 全 PASS | 初始 closeout：216 files、1,041,580 bytes、SHA-256 `cc218e25860e1236c3b2bdd0900524bac5bad1099699686750bfa63672fab5a2`；Fix 3 final 包在本报告提交后从新 clean Head 重建，manifest 校验 21 个公开 evidence 文件 / 56 个 fail-closed mutations；local `deepseek-live-failure.json` 明确排除，精确源码包 files/bytes/SHA-256 由 handoff 记录 | `PASS` |
| DeepSeek Live Canary | `npm run uat:live-deepseek`（仅 Coordinator 本地交互式执行） | 真 Key 不进入参数/history/env/inspect；固定六成员/八 Task、7 receipts、`AWAITING_OWNER_REVIEW`、zero-action；redacted evidence | 第一次 stdin transport 失败；第二次旧通用 pre-dispatch code；第三次已到 Provider broker request，但旧 task-level diagnostic 丢失具体 outcome。三次均 model receipts 0、未形成 Live验证。Fix 3 后第四次重试若失败应返回 strict `providerOutcomeCode`；Executor未接触 Owner Secret，无 Key诊断/fixture不能替代 | `PENDING / THREE FAILED ATTEMPTS / NOT LIVE_PROVIDER_VERIFIED` |
...
| AC-04 | `PASS / LIVE PENDING` | `provider-conformance.json`、provider/live-security/outcome tests、manifest negative mutations；`deepseek-live-canary.json` 待 Coordinator | 官方模型/peak 价格/URL核验；snapshot 记录 cache hit/miss/output 三项单价；flash/pro 分项费用精确；hit+miss 必须同时出现且总和等于 prompt，全部缺省按全 miss 保守计价；exact role schema 进入 Provider message/input digest；structured config/cost/latency/error、HTTP/provider/model/JSON/schema/semantic strict outcome、无换模、无 Secret。真实 Canary仍 `LIVE_PROVIDER_VERIFIED=NOT_RUN_NO_OWNER_SECRET`。 |
...
- **操作步骤：**
  ...
  3. 从页面复制三个公开标识：Organization ID、Live Mission ID、Campaign digest，依次粘回终端提示；不要粘贴 Key。启动器会在内存中生成严格四字段 JSON 并通过 captured stdin pipe 交给 Runner，不应显示 runtime bootstrap、ticket 或 request header。Runner 应自举官方 AgentTeams v1.2.0 精确六成员并只领取该 Mission；失败只显示 allowlisted envelope：`status/code/stage/providerOutcomeCode/missionId/evidence/secretPresent=false/liveProviderVerified=false`，不得出现任意异常文本。
...
- **需要返回的证据：**成功时返回 clean Head、三个公开 ID、redacted Canary JSON 的 SHA-256、终端 PASS 摘要、Mission/Review/七 receipt/零动作截图、API/浏览器 console error 摘要、`docker inspect` 的 Secret target 与 `secretInContainerEnvironment=false` 结论，以及书面 `UAT-00 PASS`。失败时返回 clean Head、三个公开 ID、终端八字段 envelope、`deepseek-live-failure.json` 的 SHA-256 与其中的 stage/code/providerOutcomeCode/progress/modelReceiptCount/noAction/cleanup 字段；绝不返回 Key、Secret 文件、Authorization header、ticket、prompt、raw model output、response ID、HTTP body 或 raw child stdout/stderr。
...
- **Coordinator 结果：**第一次 `FAIL / stdin transport defect`；第二次 `FAIL / old generic pre-dispatch code, model receipts=0`；第三次 `FAIL / PROVIDER_REQUEST, old task-level outcome hidden, model receipts=0`。Fix 3 后第四次重试 `PENDING / LIVE_PROVIDER_VERIFIED=false`；若失败必须返回且只返回 allowlisted `providerOutcomeCode`。
...
- Coordinator 在 Fix 2 Head `356a9cac79d49cbda05b5fad8855c93363159183` 的第三次真实尝试走过 runtime/project/DAG/member/dispatch 并开始首个领域 Task 的 broker request，但仍只返回顶层 `PROVIDER_REQUEST / LIVE_PROVIDER_REQUEST_FAILED`；model receipts 0、zero-action与 cleanup PASS。Coordinator独立官方 probes证明网络、credential、model access、JSON-object基础响应/identity/usage可用，但不证明产品 Task schema。Fix 3 使 Runner在清理前从精确 Mission导出 allowlisted task-level `providerOutcomeCode`，20 类 outcome各自通过真实 nested child pipe，任意 body/prompt/model content/response ID/header/ticket/bootstrap/raw异常不透传。
...
- **已发现并修复：**第三次 Canary 到达真实 broker request 后，`urllib.request.urlopen` 在 non-2xx response body解析前抛出 `HTTPError`，Runner只留下顶层 stage，cleanup后具体 provider/model/semantic outcome丢失。现在 API仅返回 stable redacted code；Runner重开 exact Mission，从 failed Task/Mission/snapshot一致性提取 allowlisted outcome。HTTP 401/402/404/429/500/502/503/504、timeout/unavailable、identity/model/finish/usage/response/JSON/schema/semantic/broker/success均有API与actual child-process测试；冲突/未知统一关闭为 `LIVE_PROVIDER_BROKER_FAILED`。
...
- **Known limitations：**第二次真实 Canary 的具体异常与第三次真实 Canary 的 task-level outcome均因当时的通用码和 cleanup不可恢复。Fix 2证明同一固定 runtime走过 dispatch；Fix 3工程验证 strict provider outcome与schema合同，但都不能替代真实 DeepSeek产品 Task。只有 Coordinator第四次带本地 Secret的成功 Canary能关闭外部门禁；若仍失败，新的 allowlisted outcome应在一次尝试中区分 infra/provider/model/schema/semantic边界。
...
- **Live UAT 恢复：**无 Key、Runner中断、Provider/schema/digest失败会持久化 `FAILED` 和下一责任人，绝不 Mock fallback。Provider非 2xx/模型/semantic失败先从精确 Mission导出 allowlisted `providerOutcomeCode`，再写 host `deepseek-live-failure.json`；随后 environment/launcher清理精确 `lumiclaw-sdd002-live-uat-cr2`、精确 AgentTeams runtime 与 0600 临时 Secret 文件并回填 cleanup。Coordinator只返回收据脱敏字段/SHA，不返回 raw日志/body。不得用全局 Docker prune。
...
- Changed Files：CR2 主要集中于 `packages/governed-shadow`、migration 000006、API Live security/server/OpenAPI、Mission/Review UI/stories/CSS、Live/Runtime Compose secrets、Coordinator launcher、Live/Mock AgentTeams verifiers；Fix 2 增加 stage diagnostics/host receipt/no-Secret六成员诊断；Fix 3修改 model-provider message/digest、API脱敏 outcome、Runner精确 Mission重读、environment传播与 provider/live/no-secret/manifest verifiers，新增 `scripts/live-provider-outcome.test.ts` 和 Change Spec lifecycle；保留既有 M2实现。
...
- Blocker：无本地工程 blocker；三次 Coordinator Canary均未产生 accepted ModelCallSnapshot，`LIVE_PROVIDER_VERIFIED=false`。第四次必须由 Coordinator用 Owner本地 Secret 独立执行；若失败，只回传 Fix 3 strict stage/providerOutcome receipt；若 PASS，再恢复 Owner UAT。该外部门禁不能由 Executor无 Key诊断、fixture 或 Mock替代。
- 下一候选步骤：Coordinator在最终 clean Head执行 UAT-00第四次重试并回传 redacted canary 或 strict failure receipt；PASS后再执行 Owner UAT-01～03与独立复核，最后决定 canonical status。本任务不自动开始 M3。

## 十一、Coordinator 验收决定

- Executor 自动化验证：`PASS / ENGINEERING_VERIFIED`（无 Secret门禁）
- Live Provider验证：`PENDING / THREE FAILED COORDINATOR ATTEMPTS / MODEL RECEIPTS 0 / NOT VERIFIED`
