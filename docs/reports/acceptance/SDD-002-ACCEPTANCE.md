...
- **Known limitations：**第二次真实 Canary 的具体异常仍因旧通用码不可恢复；第三次的历史task-level code也未保存，但相同no-Secret路径已重现loopback故障、修复并以真实Worker→API持久化结果证实。Fix 3的strict outcome/schema/no-Secret证据仍不能替代真实DeepSeek产品Task；只有Coordinator第四次带本地Secret的成功Canary能关闭外部门禁，若仍失败则新的allowlisted outcome应在一次尝试中区分provider/model/schema/semantic边界。
