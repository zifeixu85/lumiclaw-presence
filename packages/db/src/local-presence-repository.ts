    ...
    try {
      const material = await this.#database.transaction().execute(async (trx) => {
        ...
        if (duplicate !== undefined) {
          ...
          await trx.updateTable('local_onboarding_sessions').set({material_ids: JSON.stringify(ids.map((item) => item.id)), state: 'MATERIALS_READY', updated_at: now}).where('owner_profile_id', '=', input.ownerProfileId).where('path', '=', 'LOCAL_MATERIALS').executeTakeFirstOrThrow();
