...
export class PostgresKnowledgeRepository implements KnowledgeRepository {
  ...
  public async getRoleContext(ownerId: string, snapshotId: string, canonicalDigest: string): Promise<KnowledgeRoleContext> {
    ...
    for(const binding of snapshot.sourceRevisionDigests){const source=await this.#database.selectFrom('source_document_revisions as r').innerJoin('source_documents as d',(join)=>join.onRef('d.owner_profile_id','=','r.owner_profile_id').onRef('d.id','=','r.document_id')).select(['r.blob_digest','r.blob_ref','d.deleted_at']).where('r.owner_profile_id','=',ownerId).where('r.id','=',binding.revisionId).executeTakeFirst();if(source!==undefined&&source.deleted_at!==null)throw new KnowledgeContractError('SNAPSHOT_STALE');if(source===undefined||source.blob_digest.trim()!==binding.digest||!await this.blobs.has(blobRef(source.blob_ref)))throw new KnowledgeContractError('SOURCE_BLOB_MISSING');}
