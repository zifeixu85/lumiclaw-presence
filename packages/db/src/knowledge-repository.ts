...
export class PostgresKnowledgeRepository implements KnowledgeRepository {
  ...
  public async getRoleContext(ownerId: string, snapshotId: string, canonicalDigest: string): Promise<KnowledgeRoleContext> {
    const session = await this.#database.selectFrom('local_onboarding_sessions').selectAll().where('owner_profile_id', '=', ownerId).executeTakeFirst();
    if (session === undefined || session.knowledge_state !== 'KNOWLEDGE_APPROVED_NEEDS_GOAL' || session.current_knowledge_snapshot_id !== snapshotId || session.current_knowledge_snapshot_digest?.trim() !== canonicalDigest) throw new KnowledgeContractError('SNAPSHOT_STALE');
    ...
    const sources = await this.activeSourceRows(this.#database, ownerId); for (const source of sources) if (!await this.blobs.has(blobRef(source.blob_ref))) throw new KnowledgeContractError('SOURCE_BLOB_MISSING');
    if (session.target_market === null || session.knowledge_content_locale === null || session.knowledge_time_zone === null) throw new KnowledgeContractError('SNAPSHOT_STALE');
    const snapshot = snapshotFromRow(row);
    return {snapshotId, snapshotDigest: canonicalDigest, ownerId, targetMarket: session.target_market, contentLocale: session.knowledge_content_locale, timeZone: session.knowledge_time_zone, items: snapshot.itemBindings.map(({id, kind, normalizedValue, sourceRevisionIds, profileRevisionIds, ownerAuthority}) => ({id, kind, normalizedValue, sourceRevisionIds, profileRevisionIds, ownerAuthority})), sourceDigests: snapshot.sourceRevisionDigests, profileDigests: snapshot.profileRevisionDigests};
  }
  ...
  private async rebuildDraft(trx: Transaction<Database>, ownerId: string, now: Date): Promise<void> {
    ...
    await trx.updateTable('local_onboarding_sessions').set({knowledge_state: knowledgeState, current_knowledge_snapshot_id: snapshot.id, current_knowledge_snapshot_digest: snapshot.canonicalDigest}).where('owner_profile_id', '=', ownerId).executeTakeFirstOrThrow();
  }
