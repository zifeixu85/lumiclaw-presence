...
export class PostgresGoalPlanRepository implements GoalPlanRepository {
  ...
  public async appendGoal(ownerId:string,goal:OperatingGoalRevision,bindings:AccountProfileBinding[],expectedHeadDigest:string|null,guard:GoalKnowledgeGuard,idempotencyKey:string,now:Date){
    return this.withIdempotency(ownerId,'GOAL_APPEND',idempotencyKey,{goalDigest:goal.canonicalDigest,bindings,expectedHeadDigest,guard},now,async(trx)=>{
      ...
      if(head!==undefined&&head.current_digest.trim()!==goal.canonicalDigest)await this.invalidateTx(trx,ownerId,{reasonCode:'GOAL_REVISION_CHANGED',currentDigest:goal.canonicalDigest,goalId:goal.goalId},now);
  ...
  public async appendPlan(ownerId:string,plan:ContentPlanRevision,expectedHeadDigest:string|null,idempotencyKey:string,now:Date){
    return this.withIdempotency(ownerId,'PLAN_APPEND',idempotencyKey,{planDigest:plan.canonicalDigest,expectedHeadDigest},now,async(trx)=>{
      ...
      if(head!==undefined)await this.invalidateTx(trx,ownerId,{reasonCode:'PLAN_REVISION_CHANGED',currentDigest:plan.canonicalDigest,planId:plan.planId,missionIntentId:plan.missionIntentId},now);return {plan,replayed:false};
    });
  }
  ...
  public async approvePlanAndAppendBundle(ownerId:string,expectedPlanDigest:string,approvedPlan:ContentPlanRevision,bundle:MissionExecutionBundle,idempotencyKey:string,now:Date){
    return this.withIdempotency(ownerId,'PLAN_APPROVE_EXECUTION_APPEND',idempotencyKey,{expectedPlanDigest,approvedPlanDigest:approvedPlan.canonicalDigest,bundleDigest:bundle.canonicalDigest},now,async(trx)=>{
      ...
      await this.insertPlan(trx,approvedPlan,now);await trx.updateTable('content_plan_heads_v2').set({current_revision:approvedPlan.revision,current_digest:approvedPlan.canonicalDigest,row_version:head.row_version+1,state:'APPROVED',updated_at:now}).where('owner_profile_id','=',ownerId).where('plan_id','=',approvedPlan.planId).executeTakeFirstOrThrow();await this.insertBundle(trx,bundle,now);await this.invalidateTx(trx,ownerId,{reasonCode:'PLAN_REVISION_CHANGED',currentDigest:approvedPlan.canonicalDigest,planId:approvedPlan.planId,missionIntentId:approvedPlan.missionIntentId},now,bundle.bundleId);return {plan:approvedPlan,bundle,replayed:false};
    });
  }

  public async invalidateBundles(ownerId:string,request:BundleInvalidationRequest,now:Date):Promise<InvalidationEvent[]>{return this.#database.transaction().execute((trx)=>this.invalidateTx(trx,ownerId,request,now));}
  ...
  private async assertKnowledgeGuard(trx:Transaction<Database>,ownerId:string,guard:GoalKnowledgeGuard,bindings:AccountProfileBinding[]):Promise<void>{
    const session=await trx.selectFrom('local_onboarding_sessions').select('owner_profile_id').where('owner_profile_id','=',ownerId).forUpdate().executeTakeFirst();if(session===undefined)throw new GoalPlanContractError('OWNER_BOUNDARY_VIOLATION');
    const snapshot=await trx.selectFrom('knowledge_snapshots').select(['state','canonical_digest','profile_revision_digests']).where('owner_profile_id','=',ownerId).where('id','=',guard.snapshotId).executeTakeFirst();if(snapshot?.state!=='APPROVED'||snapshot.canonical_digest.trim()!==guard.snapshotDigest)throw new GoalPlanContractError('KNOWLEDGE_SNAPSHOT_NOT_APPROVED');
    const approvedProfiles=new Map(json<Array<{revisionId:string;digest:string}>>(snapshot.profile_revision_digests).map((item)=>[item.revisionId,item.digest]));
    for(const binding of bindings){const row=await trx.selectFrom('knowledge_profile_revisions').select(['digest','kind','platform_code']).where('owner_profile_id','=',ownerId).where('id','=',binding.accountProfileRevisionId).executeTakeFirst();if(row===undefined||row.kind!=='ACCOUNT'||row.platform_code!==binding.platformCode||row.digest.trim()!==binding.digest||approvedProfiles.get(binding.accountProfileRevisionId)!==binding.digest||!guard.accountProfileDigests.some((item)=>item.revisionId===binding.accountProfileRevisionId&&item.digest===binding.digest))throw new GoalPlanContractError('ACCOUNT_PROFILE_MISSING');}
  }
  ...
  private async insertBundle(trx:Transaction<Database>,bundle:MissionBundle,now:Date):Promise<void>{await trx.insertInto('mission_bundle_generations_v2').values({owner_profile_id:bundle.ownerId,bundle_id:bundle.bundleId,mission_intent_id:bundle.missionIntentId,generation:bundle.generation,kind:bundle.kind,canonical_digest:bundle.canonicalDigest,compiler_version:bundle.compilerVersion,parent_bundle_id:bundle.parentBundleId,goal_id:bundle.inputBindings.operatingGoal.id,goal_revision:bundle.inputBindings.operatingGoal.revision,goal_digest:bundle.inputBindings.operatingGoal.digest,plan_id:bundle.kind==='MISSION_EXECUTION'?bundle.approvedPlanId:null,plan_revision:bundle.kind==='MISSION_EXECUTION'?bundle.approvedPlanRevision:null,plan_digest:bundle.kind==='MISSION_EXECUTION'?bundle.approvedPlanDigest:null,payload:JSON.stringify(bundle),created_at:now}).execute();}
  private async invalidateTx(trx:Transaction<Database>,ownerId:string,request:BundleInvalidationRequest,now:Date,exceptBundleId?:string):Promise<InvalidationEvent[]>{
    let query=trx.selectFrom('mission_bundle_generations_v2').selectAll().where('owner_profile_id','=',ownerId);if(exceptBundleId!==undefined)query=query.where('bundle_id','!=',exceptBundleId);const rows=await query.execute();const events:InvalidationEvent[]=[];
    for(const row of rows){const bundle=json<MissionBundle>(row.payload);if(!matchesInvalidation(bundle,request))continue;const existing=await trx.selectFrom('mission_bundle_status_events_v2').select('event_id').where('owner_profile_id','=',ownerId).where('bundle_id','=',row.bundle_id).executeTakeFirst();if(existing!==undefined)continue;const previousDigest=previousBindingDigest(bundle,request);if(previousDigest===request.currentDigest)continue;const event:InvalidationEvent={eventId:stableContractId('invalidation',{bundleId:bundle.bundleId,reasonCode:request.reasonCode,previousDigest,currentDigest:request.currentDigest}),ownerId,bundleId:bundle.bundleId,reasonCode:request.reasonCode,previousDigest,currentDigest:request.currentDigest,recoveryAction:'REVIEW_AND_COMPILE_NEW_GENERATION',createdAt:now.toISOString()};await trx.insertInto('mission_bundle_status_events_v2').values({owner_profile_id:ownerId,event_id:event.eventId,bundle_id:event.bundleId,state:'INVALIDATED',reason_code:event.reasonCode,previous_digest:event.previousDigest,current_digest:event.currentDigest,recovery_action:event.recoveryAction,created_at:now}).execute();events.push(event);}
    return events;
  }
...
function matchesInvalidation(bundle:MissionBundle,request:BundleInvalidationRequest):boolean{if(request.reasonCode==='GOAL_REVISION_CHANGED')return bundle.inputBindings.operatingGoal.id===request.goalId;if(request.reasonCode==='PLAN_REVISION_CHANGED')return bundle.kind==='MISSION_EXECUTION'&&bundle.approvedPlanId===request.planId&&bundle.missionIntentId===request.missionIntentId;if(request.reasonCode==='KNOWLEDGE_SNAPSHOT_CHANGED')return bundle.inputBindings.knowledgeSnapshot.id===request.supersededSnapshotId&&bundle.inputBindings.knowledgeSnapshot.digest===request.supersededSnapshotDigest;if(request.reasonCode==='ACCOUNT_PROFILE_CHANGED')return bundle.inputBindings.accountProfiles.some((profile)=>request.supersededAccountProfileRevisionIds.includes(profile.accountProfileRevisionId));return bundle.missionIntentId===request.missionIntentId;}
function previousBindingDigest(bundle:MissionBundle,request:BundleInvalidationRequest):string{if(request.reasonCode==='KNOWLEDGE_SNAPSHOT_CHANGED')return bundle.inputBindings.knowledgeSnapshot.digest;if(request.reasonCode==='GOAL_REVISION_CHANGED')return bundle.inputBindings.operatingGoal.digest;if(request.reasonCode==='PLAN_REVISION_CHANGED')return bundle.kind==='MISSION_EXECUTION'?bundle.approvedPlanDigest:bundle.canonicalDigest;if(request.reasonCode==='ACCOUNT_PROFILE_CHANGED')return sha256Digest(bundle.inputBindings.accountProfiles.filter((profile)=>request.supersededAccountProfileRevisionIds.includes(profile.accountProfileRevisionId)).map((profile)=>({id:profile.accountProfileRevisionId,digest:profile.digest})));return bundle.canonicalDigest;}
