exports.up = (pgm) => {
  ...
  pgm.addConstraint('mission_bundle_generations_v2','mission_bundle_generation_shape',{check:"(kind = 'MISSION_INTENT' AND generation = 1 AND parent_bundle_id IS NULL AND plan_id IS NULL AND plan_revision IS NULL AND plan_digest IS NULL) OR (kind = 'MISSION_EXECUTION' AND generation >= 2 AND parent_bundle_id IS NOT NULL AND plan_id IS NOT NULL AND plan_revision IS NOT NULL AND plan_digest IS NOT NULL)"});
