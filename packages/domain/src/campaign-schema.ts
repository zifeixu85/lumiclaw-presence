...
const capabilitySnapshot = {
  ...
  properties: {id, organizationId: id, schemaVersion: {const: 1}, channelAccountId: id, platform, capturedAt: dateTime, expiresAt: dateTime, source: {enum: ['M1_PUBLIC_SAFE_FIXTURE', 'LOCAL_UNVERIFIED_DECLARATION']}, executionMode: {enum: ['PREPARE_ONLY', 'DIRECT_PLANNED_NOT_CONNECTED', 'NATIVE_HANDOFF_PLANNED']}, constraints: {type: 'object', minProperties: 1, additionalProperties: fieldConstraint}, disclaimer: {type: 'string', minLength: 1}}
} as const;
...
export const campaignDocumentSchema = {
  ...
  properties: {
    schemaVersion: {const: 1}, id, organizationId: id, dataMode: {enum: ['DEMO_SEED', 'LOCAL_PRIVATE']}, live: {const: false},
    ...
    evidenceRefs: {type: 'array', minItems: 1, items: {type: 'object', additionalProperties: false, required: ['id', 'organizationId', 'schemaVersion', 'label', 'sourceUrl', 'capturedAt', 'contentDigest', 'publicSafe'], properties: {id, organizationId: id, schemaVersion: {const: 1}, label: {type: 'string'}, sourceUrl: {type: 'string'}, capturedAt: dateTime, contentDigest: {type: 'string', pattern: '^[0-9a-f]{64}$'}, publicSafe: {type: 'boolean'}}}},
...
const ajv = new Ajv({allErrors: true, strict: false, formats: {'date-time': isValidRfc3339DateTime}});
