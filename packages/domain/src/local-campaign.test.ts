...
import {createLocalPrivateCampaignDocument, validateCampaignDocument, validateLocalCampaignIdentityInput, type LocalPrivateCampaignInput} from './index.js';

const input: LocalPrivateCampaignInput = {
  ...
  context: {marketCodes: ['CN', 'US'], contentLocales: ['zh-CN', 'en-US'], platforms: ['XIAOHONGSHU', 'LINKEDIN'], defaultTimeZone: 'Asia/Shanghai'},
...
describe('LOCAL_PRIVATE Campaign initialization', () => {
  it('builds a deterministic, material-bound Campaign with user-confirmed Organization, Brand, and Product authority', () => {
    ...
    expect(first.graph.markets.map((market) => market.code)).toEqual(['CN', 'US']);
    expect(first.claims[0]?.marketIds).toEqual(first.graph.markets.map((market) => market.id));
