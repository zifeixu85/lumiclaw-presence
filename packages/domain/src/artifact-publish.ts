...
export function parseProducerSubmission(value:unknown):ProducerSubmission{
  if(!exactRecord(value,['schemaVersion','evidenceMaturity','agentTeamsExecuted','submissionId','activationUnitId','producerRole','producerIdentityId','inputBindings','payload','submittedAt'])||value.schemaVersion!==3||!['CONTROLLED_FIXTURE','AGENTTEAMS_RUNTIME'].includes(String(value.evidenceMaturity))||typeof value.agentTeamsExecuted!=='boolean'||!textValue(value.submissionId)||!textValue(value.activationUnitId)||!['founder-identity-producer','product-account-producer'].includes(String(value.producerRole))||!textValue(value.producerIdentityId)||!validDateTime(value.submittedAt))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');
  return {schemaVersion:3,evidenceMaturity:value.evidenceMaturity as ProducerSubmission['evidenceMaturity'],agentTeamsExecuted:value.agentTeamsExecuted,submissionId:value.submissionId,activationUnitId:value.activationUnitId,producerRole:value.producerRole as ProducerRoleId,producerIdentityId:value.producerIdentityId,inputBindings:parseArtifactInputBindings(value.inputBindings),payload:parseArtifactPayload(value.payload),submittedAt:value.submittedAt};
}

export function parseArtifactPayload(value:unknown):ArtifactPayload{
  if(!plainRecord(value)||!['X','XIAOHONGSHU'].includes(String(value.kind)))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');
  if(value.kind==='X'){
    if(!exactRecord(value,['kind','mode','posts','link','cta','language','accountProfileRevisionId','sourceBindings'])||!['SINGLE','THREAD'].includes(String(value.mode))||!Array.isArray(value.posts)||!nullableText(value.link)||!nullableText(value.cta)||!textValue(value.language)||!textValue(value.accountProfileRevisionId))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');
    const posts=value.posts.map((post)=>{if(!exactRecord(post,['position','text','mediaRefs','altText'])||!positiveInteger(post.position)||typeof post.text!=='string'||!Array.isArray(post.mediaRefs)||!nullableText(post.altText))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');return {position:post.position,text:post.text,mediaRefs:post.mediaRefs.map(parseAuthorizedMediaRef),altText:post.altText};});
    return {kind:'X',mode:value.mode as XArtifactPayload['mode'],posts,link:value.link,cta:value.cta,language:value.language,accountProfileRevisionId:value.accountProfileRevisionId,sourceBindings:parseSourceBindings(value.sourceBindings)};
  }
  if(!exactRecord(value,['kind','title','body','topics','cta','coverSpec','imageSpecs','language','accountProfileRevisionId','sourceBindings'])||typeof value.title!=='string'||typeof value.body!=='string'||!Array.isArray(value.topics)||value.topics.some((item)=>typeof item!=='string')||!nullableText(value.cta)||!exactRecord(value.coverSpec,['purpose','aspectRatio','visualBrief','overlayCopy'])||!textValue(value.coverSpec.purpose)||!textValue(value.coverSpec.aspectRatio)||!textValue(value.coverSpec.visualBrief)||!nullableText(value.coverSpec.overlayCopy)||!Array.isArray(value.imageSpecs)||!textValue(value.language)||!textValue(value.accountProfileRevisionId))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');
  const imageSpecs=value.imageSpecs.map((spec)=>{if(!exactRecord(spec,['position','purpose','aspectRatio','visualBrief','overlayCopy','altDescription','authorizedMediaRef'])||!positiveInteger(spec.position)||!textValue(spec.purpose)||!textValue(spec.aspectRatio)||!textValue(spec.visualBrief)||!nullableText(spec.overlayCopy)||!nullableText(spec.altDescription)||(spec.authorizedMediaRef!==null&&!plainRecord(spec.authorizedMediaRef)))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');return {position:spec.position,purpose:spec.purpose,aspectRatio:spec.aspectRatio,visualBrief:spec.visualBrief,overlayCopy:spec.overlayCopy,altDescription:spec.altDescription,authorizedMediaRef:spec.authorizedMediaRef===null?null:parseAuthorizedMediaRef(spec.authorizedMediaRef)};});
  return {kind:'XIAOHONGSHU',title:value.title,body:value.body,topics:value.topics as string[],cta:value.cta,coverSpec:{purpose:value.coverSpec.purpose,aspectRatio:value.coverSpec.aspectRatio,visualBrief:value.coverSpec.visualBrief,overlayCopy:value.coverSpec.overlayCopy},imageSpecs,language:value.language,accountProfileRevisionId:value.accountProfileRevisionId,sourceBindings:parseSourceBindings(value.sourceBindings)};
}

export function parseArtifactAuditFindings(value:unknown):AuditFinding[]{
  if(!Array.isArray(value))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');const findings=value.map((finding)=>{if(!exactRecord(finding,['checkCode','result','path','message','evidenceBindings','recoveryAction'])||!auditCheckCodes.includes(finding.checkCode as AuditCheckCode)||!['PASS','FAIL','ESCALATE'].includes(String(finding.result))||!textValue(finding.path)||!textValue(finding.message)||!Array.isArray(finding.evidenceBindings)||finding.evidenceBindings.some((item)=>!isDigest(item))||!nullableText(finding.recoveryAction)||(finding.result!=='PASS'&&finding.recoveryAction===null))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');return {checkCode:finding.checkCode as AuditCheckCode,result:finding.result as AuditResult,path:finding.path,message:finding.message,evidenceBindings:[...finding.evidenceBindings] as string[],recoveryAction:finding.recoveryAction};});
  if(findings.length>0&&!hasExactAuditFindingSet(findings))throw new ArtifactContractError('AUDIT_FINDINGS_INCOMPLETE');return findings;
}
...
export function createArtifactAuditDecision(input:{ownerId:string;revision:ArtifactRevisionV3;auditorIdentityId:string;result:AuditResult;findings:AuditFinding[];evidenceBindings:string[];createdAt:string}):ArtifactAuditDecision{
  ...
  const findings=parseArtifactAuditFindings(input.findings);if(!hasExactAuditFindingSet(findings))throw new ArtifactContractError('AUDIT_FINDINGS_INCOMPLETE');
  const derived=findings.some((item)=>item.result==='ESCALATE')?'ESCALATE':findings.some((item)=>item.result==='FAIL')?'FAIL':'PASS';if(derived!==input.result)throw new ArtifactContractError('AUDIT_FINDINGS_INCOMPLETE');
  const evidenceBindings=parseDigestList(input.evidenceBindings);const policyDigest=artifactSkills.AUDIT.digest;const base={schemaVersion:3 as const,id:stableId('artifact-audit',{revision:input.revision.canonicalDigest,auditor:input.auditorIdentityId,result:input.result,findings}),ownerId:input.ownerId,artifactRevisionId:input.revision.id,artifactRevisionDigest:input.revision.canonicalDigest,auditorRole:'independent-auditor' as const,auditorIdentityId:boundedText(input.auditorIdentityId,1,200,'ARTIFACT_SCHEMA_INVALID'),result:input.result,findings,evidenceBindings:uniqueSorted(evidenceBindings),policyVersion:'artifact-independent-audit@1.0.0' as const,policyDigest,createdAt:input.createdAt};return {...base,canonicalDigest:sha256Digest(base)};
}
...
export function validateArtifactAuditAuthority(value:unknown,revision:ArtifactRevisionV3):ArtifactAuditDecision{
  if(!exactRecord(value,['schemaVersion','id','ownerId','artifactRevisionId','artifactRevisionDigest','auditorRole','auditorIdentityId','result','findings','evidenceBindings','policyVersion','policyDigest','canonicalDigest','createdAt'])||value.schemaVersion!==3||!textValue(value.id)||!textValue(value.ownerId)||!textValue(value.artifactRevisionId)||!isDigest(value.artifactRevisionDigest)||value.auditorRole!=='independent-auditor'||!textValue(value.auditorIdentityId)||!['PASS','FAIL','ESCALATE'].includes(String(value.result))||!Array.isArray(value.evidenceBindings)||value.evidenceBindings.some((item)=>!isDigest(item))||value.policyVersion!=='artifact-independent-audit@1.0.0'||!isDigest(value.policyDigest)||!isDigest(value.canonicalDigest)||!validDateTime(value.createdAt))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');
  if(value.ownerId!==revision.ownerId||value.artifactRevisionId!==revision.id||value.artifactRevisionDigest!==revision.canonicalDigest)throw new ArtifactContractError('OWNER_DECISION_STALE');if(value.auditorIdentityId===revision.producerIdentityId)throw new ArtifactContractError('AUDITOR_INDEPENDENCE_REQUIRED');
  const findings=parseArtifactAuditFindings(value.findings);const expected=createArtifactAuditDecision({ownerId:revision.ownerId,revision,auditorIdentityId:value.auditorIdentityId,result:value.result as AuditResult,findings,evidenceBindings:value.evidenceBindings as string[],createdAt:value.createdAt});if(value.id!==expected.id||value.policyDigest!==expected.policyDigest||value.canonicalDigest!==expected.canonicalDigest)throw new ArtifactContractError('ARTIFACT_INPUT_MISMATCH');return structuredClone(value) as ArtifactAuditDecision;
}

export function validateArtifactOwnerDecisionAuthority(value:unknown,revision:ArtifactRevisionV3,audit:ArtifactAuditDecision):ArtifactOwnerDecision{
  if(!exactRecord(value,['schemaVersion','id','ownerId','artifactRevisionId','artifactRevisionDigest','auditDecisionId','auditDecisionDigest','result','ownerIdentityId','reason','canonicalDigest','decidedAt'])||value.schemaVersion!==3||!textValue(value.id)||!textValue(value.ownerId)||!textValue(value.artifactRevisionId)||!isDigest(value.artifactRevisionDigest)||!textValue(value.auditDecisionId)||!isDigest(value.auditDecisionDigest)||!['APPROVE','REJECT'].includes(String(value.result))||!textValue(value.ownerIdentityId)||!nullableText(value.reason)||!isDigest(value.canonicalDigest)||!validDateTime(value.decidedAt))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');
  if(value.ownerId!==revision.ownerId||audit.ownerId!==revision.ownerId||value.artifactRevisionId!==revision.id||value.artifactRevisionDigest!==revision.canonicalDigest||value.auditDecisionId!==audit.id||value.auditDecisionDigest!==audit.canonicalDigest||audit.artifactRevisionId!==revision.id||audit.artifactRevisionDigest!==revision.canonicalDigest)throw new ArtifactContractError('OWNER_DECISION_STALE');if(value.result==='APPROVE'&&audit.result!=='PASS')throw new ArtifactContractError('AUDIT_PASS_REQUIRED');
  const expected=createArtifactOwnerDecision({ownerId:revision.ownerId,revision,audit,result:value.result as OwnerArtifactDecisionResult,ownerIdentityId:value.ownerIdentityId,reason:value.reason as string|null,decidedAt:value.decidedAt});if(value.id!==expected.id||value.canonicalDigest!==expected.canonicalDigest)throw new ArtifactContractError('ARTIFACT_INPUT_MISMATCH');return structuredClone(value) as ArtifactOwnerDecision;
}
...
export function buildManualPublishPackage(input:{ownerId:string;revision:ArtifactRevisionV3;audit:ArtifactAuditDecision;decision:ArtifactOwnerDecision;createdAt:string;officialUrlRef?:OfficialPublishUrlRef}):ManualPublishPackage{
  ...
  if(audit.result!=='PASS'||decision.result!=='APPROVE')throw new ArtifactContractError('AUDIT_PASS_REQUIRED');if(audit.artifactRevisionId!==revision.id||audit.artifactRevisionDigest!==revision.canonicalDigest||decision.artifactRevisionId!==revision.id||decision.artifactRevisionDigest!==revision.canonicalDigest||decision.auditDecisionId!==audit.id||decision.auditDecisionDigest!==audit.canonicalDigest)throw new ArtifactContractError('PACKAGE_INPUT_INVALIDATED');
...
export function validateManualPublishPackageAuthority(value:unknown,revision:ArtifactRevisionV3,audit:ArtifactAuditDecision,decision:ArtifactOwnerDecision):ManualPublishPackage{
  if(!exactRecord(value,['schemaVersion','id','ownerId','platformCode','accountProfileRevisionId','artifactRevisionId','artifactRevisionDigest','auditDecisionId','auditDecisionDigest','ownerDecisionId','ownerDecisionDigest','officialPublishUrlRef','exactInputDigest','orderedFiles','manifestDigest','state','createdAt','externalState'])||value.schemaVersion!==3||!textValue(value.id)||!textValue(value.ownerId)||!artifactPlatforms.includes(value.platformCode as ArtifactPlatform)||!textValue(value.accountProfileRevisionId)||!textValue(value.artifactRevisionId)||!isDigest(value.artifactRevisionDigest)||!textValue(value.auditDecisionId)||!isDigest(value.auditDecisionDigest)||!textValue(value.ownerDecisionId)||!isDigest(value.ownerDecisionDigest)||!isDigest(value.exactInputDigest)||!isDigest(value.manifestDigest)||value.state!=='READY'||value.externalState!=='UNVERIFIED_EXTERNAL_STATE'||!validDateTime(value.createdAt)||!Array.isArray(value.orderedFiles)||!validOfficialRefShape(value.officialPublishUrlRef))throw new ArtifactContractError('PACKAGE_INPUT_INVALIDATED');
  if(value.orderedFiles.some((entry)=>!exactRecord(entry,['position','fileName','mediaType','content','digest'])||!positiveInteger(entry.position)||!textValue(entry.fileName)||!['application/json','text/plain','text/markdown'].includes(String(entry.mediaType))||typeof entry.content!=='string'||!isDigest(entry.digest)))throw new ArtifactContractError('PACKAGE_INPUT_INVALIDATED');
  if(value.ownerId!==revision.ownerId||audit.ownerId!==revision.ownerId||decision.ownerId!==revision.ownerId||value.platformCode!==revision.platformCode||value.accountProfileRevisionId!==revision.inputBindings.accountProfile.id||value.artifactRevisionId!==revision.id||value.artifactRevisionDigest!==revision.canonicalDigest||value.auditDecisionId!==audit.id||value.auditDecisionDigest!==audit.canonicalDigest||value.ownerDecisionId!==decision.id||value.ownerDecisionDigest!==decision.canonicalDigest||audit.artifactRevisionId!==revision.id||audit.artifactRevisionDigest!==revision.canonicalDigest||decision.artifactRevisionId!==revision.id||decision.artifactRevisionDigest!==revision.canonicalDigest||decision.auditDecisionId!==audit.id||decision.auditDecisionDigest!==audit.canonicalDigest)throw new ArtifactContractError('PACKAGE_INPUT_INVALIDATED');
  const expected=buildManualPublishPackage({ownerId:revision.ownerId,revision,audit,decision,createdAt:value.createdAt,officialUrlRef:value.officialPublishUrlRef as OfficialPublishUrlRef});if(sha256Digest(value)!==sha256Digest(expected))throw new ArtifactContractError('PACKAGE_INPUT_INVALIDATED');return structuredClone(value) as ManualPublishPackage;
}
...
function parseArtifactInputBindings(value:unknown):ArtifactInputBindings{
  if(!exactRecord(value,['executionBundle','operatingGoal','approvedPlan','knowledgeSnapshot','accountProfile','producerSkill','artifactProfile','sourceSetDigest'])||!exactRecord(value.executionBundle,['id','digest','generation','missionIntentId'])||!textValue(value.executionBundle.id)||!isDigest(value.executionBundle.digest)||!positiveInteger(value.executionBundle.generation)||!textValue(value.executionBundle.missionIntentId)||!idDigestRef(value.operatingGoal)||!idDigestRef(value.approvedPlan)||!idDigestRef(value.knowledgeSnapshot)||!exactRecord(value.accountProfile,['id','digest','platformCode'])||!textValue(value.accountProfile.id)||!isDigest(value.accountProfile.digest)||!artifactPlatforms.includes(value.accountProfile.platformCode as ArtifactPlatform)||!validSkillRefShape(value.producerSkill)||!validProfileRefShape(value.artifactProfile)||!isDigest(value.sourceSetDigest))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');
  return structuredClone(value) as ArtifactInputBindings;
}
function parseAuthorizedMediaRef(value:unknown):AuthorizedMediaRef{if(!exactRecord(value,['digest','fileName','mediaType','rights','altText'])||!isDigest(value.digest)||!textValue(value.fileName)||!['image/png','image/jpeg','image/webp'].includes(String(value.mediaType))||value.rights!=='OWNER_AUTHORIZED_LOCAL'||!textValue(value.altText))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');return structuredClone(value) as AuthorizedMediaRef;}
function parseSourceBindings(value:unknown):ArtifactSourceBinding[]{if(!Array.isArray(value))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');return value.map((binding)=>{if(!exactRecord(binding,['sourceItemId','bindingDigest'])||!textValue(binding.sourceItemId)||!isDigest(binding.bindingDigest))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');return {sourceItemId:binding.sourceItemId,bindingDigest:binding.bindingDigest};});}
function parseDigestList(value:unknown):string[]{if(!Array.isArray(value)||value.some((item)=>!isDigest(item)))throw new ArtifactContractError('ARTIFACT_SCHEMA_INVALID');return [...value] as string[];}
function validSkillRefShape(value:unknown):boolean{return exactRecord(value,['id','version','digest','source','sourceDigest','license'])&&['x-content-expression','xiaohongshu-content-expression','artifact-independent-audit'].includes(String(value.id))&&value.version==='1.0.0'&&isDigest(value.digest)&&typeof value.source==='string'&&/^skills\/[A-Za-z0-9._-]+\/SKILL\.md$/u.test(value.source)&&isDigest(value.sourceDigest)&&value.license==='Apache-2.0';}
function validProfileRefShape(value:unknown):boolean{return exactRecord(value,['id','version','digest','platformCode','source','checkedAt','expiresAt'])&&textValue(value.id)&&value.version==='1.0.0'&&isDigest(value.digest)&&artifactPlatforms.includes(value.platformCode as ArtifactPlatform)&&textValue(value.source)&&validDateTime(value.checkedAt)&&validDateTime(value.expiresAt);}
function validOfficialRefShape(value:unknown):boolean{return exactRecord(value,['id','platformCode','url','sourceId','source','sourceVersion','checkedAt','expiresAt','digest'])&&textValue(value.id)&&artifactPlatforms.includes(value.platformCode as ArtifactPlatform)&&textValue(value.url)&&textValue(value.sourceId)&&textValue(value.source)&&textValue(value.sourceVersion)&&validDateTime(value.checkedAt)&&validDateTime(value.expiresAt)&&isDigest(value.digest);}
function idDigestRef(value:unknown):boolean{return exactRecord(value,['id','digest'])&&textValue(value.id)&&isDigest(value.digest);}
...
function exactRecord(value:unknown,keys:string[]):value is Record<string,unknown>{return plainRecord(value)&&Object.keys(value).sort().join(',')===[...keys].sort().join(',');}
function textValue(value:unknown):value is string{return typeof value==='string'&&value.length>0;}
...
function validDateTime(value:unknown):value is string{return typeof value==='string'&&!Number.isNaN(Date.parse(value));}
...
function hasExactAuditFindingSet(findings:AuditFinding[]):boolean{const codes=new Set(findings.map((item)=>item.checkCode));return findings.length===auditCheckCodes.length&&codes.size===auditCheckCodes.length&&auditCheckCodes.every((code)=>codes.has(code));}
